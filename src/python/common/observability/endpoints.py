...
async def metrics_prometheus() -> str:
    ...
    try:
        ...
        logger.debug(
            "Metrics exported", format="prometheus", size_bytes=len(prometheus_data)
        )
...
def add_observability_routes(app: FastAPI, prefix: str = "") -> None:
    ...
    async def health_endpoint(request: Request) -> JSONResponse:
        ...
        try:
            ...
            metrics_instance.increment_counter(
                "health_endpoint_requests_total", status=health_data["status"]
            )
            ...
            metrics_instance.record_histogram(
                "health_endpoint_duration_seconds", response_time
            )
            ...
            metrics_instance.increment_counter(
                "health_endpoint_requests_total", status="error"
            )
    ...
    async def health_detailed_endpoint(request: Request) -> JSONResponse:
        ...
        try:
            ...
            # Record metrics
            metrics_instance.increment_counter(
                "health_detailed_requests_total", status=health_data["status"]
            )

            response_time = time.perf_counter() - start_time
            metrics_instance.record_histogram(
                "health_detailed_duration_seconds", response_time
            )
            ...
            metrics_instance.increment_counter(
                "health_detailed_requests_total", status="error"
            )
    ...
    async def metrics_endpoint(request: Request) -> PlainTextResponse:
        ...
        try:
            ...
            response_time = time.perf_counter() - start_time
            metrics_instance.record_histogram(
                "metrics_endpoint_duration_seconds", response_time
            )
            metrics_instance.increment_counter(
                "metrics_endpoint_requests_total", format="prometheus"
            )
...
"""
    ...
    async def metrics_json_endpoint(request: Request) -> JSONResponse:
        ...
        try:
            ...
            metrics_instance.increment_counter(
                "metrics_endpoint_requests_total", format="json"
            )
            ...
            metrics_instance.increment_counter(
                "metrics_endpoint_requests_total", format="json", status="error"
            )
    ...
    async def diagnostics_endpoint(request: Request) -> JSONResponse:
        ...
        try:
            ...
            response_time = time.perf_counter() - start_time
            metrics_instance.record_histogram(
                "diagnostics_endpoint_duration_seconds", response_time
            )
            ...
            metrics_instance.increment_counter(
                "diagnostics_endpoint_requests_total", status="error"
            )
    ...
    # Add startup logging
    logger.info(
        "Observability endpoints registered",
        routes=[
            f"{prefix}/health",
            f"{prefix}/health/detailed",
            f"{prefix}/metrics",
...
def setup_observability_middleware(app: FastAPI) -> None:
    ...
    async def observability_middleware(request: Request, call_next):
        ...
        try:
            ...
            metrics_instance.record_histogram(
                "http_request_duration_seconds", response_time, method=method, path=path
            )
            ...
            metrics_instance.increment_counter(
                "http_requests_total", method=method, status_code="500"
            )
            metrics_instance.record_histogram(
                "http_request_duration_seconds", response_time, method=method, path=path
            )
