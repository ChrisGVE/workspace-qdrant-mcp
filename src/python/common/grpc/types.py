...
class ProcessDocumentResponse:
    ...
    @classmethod
    def from_pb(
        cls, pb_response: PbProcessDocumentResponse
    ) -> "ProcessDocumentResponse":
        """Create from protobuf message."""
        return cls(
            success=pb_response.success,
            message=pb_response.message,
            document_id=pb_response.document_id
            if pb_response.HasField("document_id")
            else None,
            chunks_added=pb_response.chunks_added,
            applied_metadata=dict(pb_response.applied_metadata)
            if pb_response.applied_metadata
            else None,
        )
...
class HealthCheckResponse:
    ...
    def from_pb(cls, pb_response: PbHealthResponse) -> "HealthCheckResponse":
        ...
        return cls(status=status_str, message=pb_response.message, services=services)
