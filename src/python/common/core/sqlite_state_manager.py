...
class ContentIngestionStatus(Enum):
    """Status for content-based ingestion queue items."""
...
class ContentIngestionOperation(Enum):
    """Operation type for content ingestion."""
...
class ContentIngestionQueueItem:
    """Item in the content-based ingestion queue.

    Used for queueing content writes when daemon is unavailable.
    The daemon processes this queue when it becomes available.

    Attributes:
        queue_id: Unique identifier (UUID)
        operation: Operation type (create/modify/delete)
        target_collection: Target collection name (canonical: memory, projects, libraries)
        source_type: Content source type (e.g., "text", "memory", "note")
        source_id: Document identifier (path or ID)
        main_tag: Primary tag for filtering
        full_tag: Full hierarchical tag
        content: The actual content to ingest
        metadata: Additional metadata (JSON)
        priority: Processing priority (0-10)
        status: Queue item status
        idempotency_key: Hash of content+metadata for deduplication
        retry_count: Number of retry attempts
        error_message: Last error message if failed
        created_at: When the item was queued
        updated_at: Last update timestamp
    """

    queue_id: str
    operation: ContentIngestionOperation
    ...
    def _generate_idempotency_key(self) -> str:
        """Generate idempotency key from content hash + metadata."""
        key_data = f"{self.content}:{self.target_collection}:{self.source_type}"
        if self.metadata:
            key_data += f":{json.dumps(self.metadata, sort_keys=True)}"
...
class SQLiteStateManager:
    ...
    async def _create_initial_schema(self):
        ...
        schema_sql = [
            ...
            # Content ingestion queue for daemon-unavailable fallback (v8)
            """
            ...
            "CREATE INDEX idx_content_ingestion_queue_status ON content_ingestion_queue(status)",
    ...
    async def _migrate_schema(self, from_version: int, to_version: int):
        ...
        async with self.transaction() as conn:
            ...
            # Migrate from version 7 to version 8 - Add content ingestion queue
            if from_version <= 7 and to_version >= 8:
                logger.info("Applying migration: v7 -> v8 (content ingestion queue)")
                content_queue_sql = [
                    # Add content_ingestion_queue table for daemon-unavailable fallback
                    """
                    ...
                    """,
                    # Add indexes for content_ingestion_queue
                    "CREATE INDEX IF NOT EXISTS idx_content_ingestion_queue_status ON content_ingestion_queue(status)",
                    "CREATE INDEX IF NOT EXISTS idx_content_ingestion_queue_priority ON content_ingestion_queue(priority DESC, created_at ASC)",
                    "CREATE INDEX IF NOT EXISTS idx_content_ingestion_queue_collection ON content_ingestion_queue(target_collection)",
                    "CREATE INDEX IF NOT EXISTS idx_content_ingestion_queue_idempotency ON content_ingestion_queue(idempotency_key)",
                ]
    ...
    # Content-based Ingestion Queue Methods (for daemon-unavailable fallback)
    ...
    async def enqueue_ingestion(
        ...
        """
        Enqueue content for ingestion when daemon is unavailable.

        This method queues content writes to SQLite for later processing
        by the daemon, replacing direct Qdrant fallback writes.

        Args:
            content: The text content to ingest
            target_collection: Target canonical collection (memory, projects, libraries)
            source_type: Content source type (e.g., "text", "memory", "note")
            operation: Operation type ("create", "modify", "delete")
            ...
            priority: Priority level (0-10, where 10 is highest)
                     Active projects use HIGH (8), background uses NORMAL (5)

        Returns:
            Queue ID (UUID) of the enqueued item

        Raises:
            ValueError: If priority is out of valid range or operation is invalid
            RuntimeError: If state manager not initialized
        """
        import uuid
        ...
        try:
            ...
            key_data = f"{content}:{target_collection}:{source_type}"
            if metadata:
                key_data += f":{json.dumps(metadata, sort_keys=True)}"
            idempotency_key = hashlib.sha256(key_data.encode("utf-8")).hexdigest()[:32]

            async with self.transaction() as conn:
                # Check for existing item with same idempotency key
                cursor = conn.execute(
                    """
                    SELECT queue_id, status FROM content_ingestion_queue
                ...
                if existing:
                    ...
                    if existing_status in ("pending", "in_progress"):
                        ...
                        logger.debug(
                            f"Updated existing queue item {existing_id} (idempotency match)"
                        )
                        return existing_id
                    elif existing_status == "done":
                        # Already processed, skip
                        logger.debug(
                            f"Content already processed (queue_id={existing_id})"
                        )
                        return existing_id
                    else:
                        # Failed item, retry with new queue_id
                        pass
                ...
                conn.execute(
                    ...
                    (queue_id, operation, target_collection, source_type, source_id,
                     main_tag, full_tag, content, metadata, priority, status, idempotency_key)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)
                    """,
                    (
                        queue_id,
                ...
                logger.info(
                    f"Queued content for ingestion: queue_id={queue_id}, "
                    f"collection={target_collection}, priority={priority}"
                )
                return queue_id
            ...
            logger.error(f"Failed to enqueue content: {e}")
            raise
    ...
    async def dequeue_content_ingestion(
        ...
        """
        Retrieve pending items from content ingestion queue.

        Args:
            batch_size: Maximum number of items to retrieve
            target_collection: Optional filter by target collection

        Returns:
            List of ContentIngestionQueueItem objects
        """
        if not self._initialized:
            raise RuntimeError("State manager not initialized")
        ...
        try:
            query = """
                SELECT queue_id, operation, target_collection, source_type, source_id,
                       main_tag, full_tag, content, metadata, priority, status,
                       idempotency_key, retry_count, error_message, created_at, updated_at
            ...
            with self._lock:
                ...
                items = []
                for row in rows:
                    items.append(ContentIngestionQueueItem(
                        queue_id=row["queue_id"],
                        operation=ContentIngestionOperation(row["operation"]),
                        ...
                        status=ContentIngestionStatus(row["status"]),
    ...
    async def update_content_ingestion_status(
        ...
        try:
            async with self.transaction() as conn:
                ...
                if updated:
                    logger.debug(f"Updated content queue item {queue_id} status to {status}")
                else:
                    logger.warning(f"Content queue item not found: {queue_id}")

                return updated

        except Exception as e:
            logger.error(f"Failed to update content queue status: {e}")
            raise
    ...
    async def get_content_ingestion_queue_depth(
        ...
        """
        Get the current depth of the content ingestion queue.

        Args:
            status: Optional filter by status
            target_collection: Optional filter by collection
        ...
        try:
            ...
            logger.error(f"Failed to get content queue depth: {e}")
            raise

    async def remove_from_content_queue(self, queue_id: str) -> bool:
        """
        Remove an item from the content ingestion queue.
        ...
        try:
            async with self.transaction() as conn:
                cursor = conn.execute(
                    "DELETE FROM content_ingestion_queue WHERE queue_id = ?",
                    (queue_id,)
                )
                deleted = cursor.rowcount > 0

                if deleted:
                    logger.debug(f"Removed from content queue: {queue_id}")
                else:
                    logger.warning(f"Content queue item not found: {queue_id}")
            ...
            logger.error(f"Failed to remove from content queue: {e}")
            raise

    async def reset_in_progress_content_items(self) -> int:
        """
        Reset all in_progress items back to pending (for crash recovery).
        ...
        try:
            async with self.transaction() as conn:
                ...
                if count > 0:
                    logger.info(f"Reset {count} in_progress content items to pending")

                return count

        except Exception as e:
            logger.error(f"Failed to reset in_progress content items: {e}")
            raise
