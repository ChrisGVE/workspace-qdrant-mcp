...
class LSPConfig:
    ...
    def _apply_env_overrides(self) -> None:
        ...
        if os.getenv(f"{prefix}_ENABLED_LSPS"):
            ...
            self.enabled_lsps = {lsp.strip() for lsp in enabled_str.split(",") if lsp.strip()}

        if os.getenv(f"{prefix}_DISABLED_LSPS"):
            disabled_str = os.getenv(f"{prefix}_DISABLED_LSPS", "")
            self.disabled_lsps = {lsp.strip() for lsp in disabled_str.split(",") if lsp.strip()}
    ...
    def get_lsp_binary_path(self, lsp_name: str) -> str | None:
    ...
    @classmethod
    def load_from_file(cls, config_file: str | Path) -> "LSPConfig":
...
class LSPCache:
    ...
    def __init__(self, cache_file: str | Path | None = None, default_ttl: int = 300):
        """
    ...
    def get(self, lsp_name: str, ttl: int | None = None) -> LSPCacheEntry | None:
...
_default_config: LSPConfig | None = None
_default_cache: LSPCache | None = None
_config_lock = threading.RLock()


def get_default_config(config_file: str | Path | None = None) -> LSPConfig:
...
def get_default_cache(cache_file: str | Path | None = None) -> LSPCache:
