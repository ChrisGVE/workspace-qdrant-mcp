...
class FileLockManager:
    ...
    async def _acquire_lock(self) -> None:
        ...
        if not lock_acquired:
            raise TimeoutError(
                f"Could not acquire lock on {self.config_file} within {self._lock_timeout} seconds"
            )
...
class WatchEventNotifier:
    ...
    async def _process_event(self, event: ConfigChangeEvent) -> None:
        ...
        logger.debug(
            f"Processing config change event: {event.event_type} for {event.watch_id}"
        )
    ...
    def get_event_history(
        self, watch_id: Optional[str] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
...
class SynchronizedWatchConfigManager:
    ...
    def __init__(
        self, config_file: Optional[Path] = None, project_dir: Optional[Path] = None
    ):
    ...
    async def add_watch_config(
        self, watch_config: Any, source: str = "mcp_tool"
    ) -> bool:
        ...
        if success:
            ...
            logger.info(
                f"Watch config {'added' if not existing_watch else 'modified'}: {watch_config.id}"
            )

        return success

    async def remove_watch_config(
        self, watch_id: str, source: str = "mcp_tool"
    ) -> bool:
    ...
    async def update_watch_status(
        self, watch_id: str, status: str, source: str = "system"
    ) -> bool:
