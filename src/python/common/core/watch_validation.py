...
class WatchPathValidator:
    ...
    def validate_permissions(path: Path) -> ValidationResult:
        ...
        try:
            ...
            try:
                ...
                warnings.append(
                    "Cannot list directory contents - may affect file discovery"
                )
    ...
    def validate_path_type(path: Path) -> ValidationResult:
        ...
        try:
            ...
            path_str = str(path)
            if (
                path_str.startswith(("\\\\\\\\", "//", "smb://", "nfs://"))
                or "://" in path_str
            ):
                warnings.append(
                    "Path appears to be a network location - may have reliability issues"
                )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    warnings.append(
                        "Cannot create test files - watch reliability may be affected"
                    )
...
class WatchErrorRecovery:
    ...
    async def attempt_recovery(
        self, watch_id: str, error_type: str, path: Path, error_details: str = ""
    ) -> Tuple[bool, str]:
        ...
        if attempt_count >= self.max_recovery_attempts:
            return (
                False,
                f"Maximum recovery attempts ({self.max_recovery_attempts}) exceeded for {error_type}",
            )
        ...
        # Apply backoff delay
        if attempt_count > 0:
            backoff_delay = self.recovery_backoff_seconds[
                min(attempt_count - 1, len(self.recovery_backoff_seconds) - 1)
            ]
            logger.info(
                f"Waiting {backoff_delay} seconds before recovery attempt {attempt_count + 1}"
            )
            await asyncio.sleep(backoff_delay)

        # Attempt recovery
        success, details = await self._execute_recovery_strategy(
            recovery_strategy, path, error_details
        )
        ...
        logger.info(
            f"Recovery attempt for {watch_id} ({error_type}): {'SUCCESS' if success else 'FAILED'} - {details}"
        )
    ...
    async def _execute_recovery_strategy(
        self, strategy: Dict[str, Any], path: Path, error_details: str
    ) -> Tuple[bool, str]:
        ...
        try:
            ...
            if action == "wait_and_validate":
                ...
                if result.valid:
                    ...
                    return (
                        False,
                        f"Symbolic link issues persist: {result.error_message}",
                    )
    ...
    async def _monitoring_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                await asyncio.sleep(
                    min(self.monitoring_interval, 60)
            ...
            try:
                ...
                if validation_result.valid:
                    ...
                    if health_info["consecutive_failures"] >= 3:
                        logger.info(
                            f"Triggering recovery for watch {watch_id} after {health_info['consecutive_failures']} failures"
                        )
                        ...
                        if success:
                            ...
                            logger.info(
                                f"Recovery successful for watch {watch_id}: {details}"
                            )
                        else:
                            logger.error(
                                f"Recovery failed for watch {watch_id}: {details}"
                            )
