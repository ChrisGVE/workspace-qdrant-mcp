...
class ConfigManager:
    ...
    def set(self, path: str, value: Any) -> bool:
        ...
        This modifies the in-memory configuration only. Use save_to_file() to
        persist changes to disk.

        Args:
            path: Dot-separated configuration path (e.g., "qdrant.url", "server.debug")
            value: Value to set (must match expected type for that path)
    ...
    def get_config_file_path(self) -> Path | None:
        ...
        Returns:
            Path to the configuration file, or None if using defaults only
        """
        return self._find_default_config_file()

    # Settings that require a daemon/server restart when changed
    ...
    @classmethod
    def requires_restart(cls, setting_path: str) -> bool:
        """Check if changing a setting requires a server/daemon restart.

        Args:
            setting_path: Dot-separated configuration path

        Returns:
            bool: True if changing this setting requires a restart
        """
        # Check exact match
        if setting_path in cls.RESTART_REQUIRED_SETTINGS:
            return True

        # Check if it's a child of a restart-required section
        for required_setting in cls.RESTART_REQUIRED_SETTINGS:
            if setting_path.startswith(required_setting + "."):
                return True
