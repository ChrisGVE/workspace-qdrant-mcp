...
class HtmlParser(DocumentParser):
    ...
    async def parse(self, file_path: str | Path, **options: Any) -> ParsedDocument:
        ...
        try:
            ...
            logger.info(
                f"Successfully parsed HTML: {file_path.name} "
    ...
    async def _read_html_file(
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to decode with {detected_encoding}, using UTF-8 fallback"
                )
    ...
    def _detect_encoding(self, content: bytes) -> str:
        ...
        try:
            ...
            charset_match = re.search(
                r'<meta\s+charset\s*=\s*["\']?([^"\'\s>]+)', content_str
            )
            if charset_match:
    ...
    async def _extract_text_content(
        ...
        # Remove comments
        for comment in content_soup.find_all(
        ...
        if content_selectors:
            ...
            main_content = (
                ...
                or content_soup.find('[role="main"]')
                or content_soup.find("#main")
                or content_soup.find("#content")
                or content_soup.find(".main")
                or content_soup.find(".content")
            )
            if main_content:
