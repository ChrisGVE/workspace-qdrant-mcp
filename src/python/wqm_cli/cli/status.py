...
# SQLite state management for local queries (Task 422)
from common.core.sqlite_state_manager import SQLiteStateManager

# Global state manager instance for status queries
...
async def get_processing_status() -> dict:
    """Get processing status from SQLite state manager."""
    try:
        state_mgr = await _get_state_manager()
        queue_depth = await state_mgr.get_queue_depth()
        content_queue_depth = await state_mgr.get_content_ingestion_queue_depth()
        return {
            "success": True,
            "queue_depth": queue_depth,
            "content_queue_depth": content_queue_depth,
            "status": "active" if queue_depth > 0 or content_queue_depth > 0 else "idle",
        }
    except Exception as e:
        logger.debug(f"Error getting processing status: {e}")
...
async def get_queue_stats() -> dict:
    ...
    try:
        ...
        return {
            "success": True,
            "file_queue_size": queue_depth,
            "content_queue_size": content_queue_depth,
            "total_queue_size": queue_depth + content_queue_depth,
        }
    except Exception as e:
...
async def get_watch_folder_configs() -> dict:
    """Get watch folder configurations from SQLite state manager."""
    try:
        state_mgr = await _get_state_manager()
        configs = await state_mgr.list_watch_folders()
        return {
            "success": True,
            "configs": configs,
            "count": len(configs),
        ...
        logger.debug(f"Error getting watch configs: {e}")
        return {"success": False, "error": str(e)}
...
async def get_database_stats() -> dict:
    ...
    try:
        ...
        return {"success": True, "database_path": "in-memory", "database_size_mb": 0}
    except Exception as e:
...
async def get_comprehensive_status() -> dict[str, Any]:
    ...
    Task 422: Uses SQLiteStateManager for local state queries and
    gRPC for daemon communication.
    """
    try:
        # Gather data in parallel from SQLite and gRPC
        tasks = []

        # Processing status from SQLite
        tasks.append(get_processing_status())

        # Queue statistics from SQLite
        tasks.append(get_queue_stats())

        # Watch folder configurations from SQLite
        tasks.append(get_watch_folder_configs())

        # Database statistics from SQLite
        tasks.append(get_database_stats())
