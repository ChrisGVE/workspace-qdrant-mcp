...
//! accessor methods.  Every RPC call made through this client is protected by:
//!
//! 1. **Client-side timeout** via `tokio::time::timeout` (see below).
...
//! connection.ts lines 102-121): the timer fires and rejects the outer promise,
//! but the underlying gRPC call is **abandoned in place** — no cancellation
//! signal is sent to the server and **no `grpc-timeout` header** is written to
//! the request metadata (AC-DC4 / RISK-7).
//!
//! Tonic's `.timeout(d)` on a `Channel` behaves differently: it sets the
//! `grpc-timeout` header on every request, which the server uses to cancel its
//! own work.  To preserve the TS client-side-abandon semantics we wrap each
//! call with `tokio::time::timeout(...)` instead.
//!
//! # Self-healing / ensureConnected
//!
//! Because we use `connect_lazy()` the channel reconnects automatically on the
//! next RPC after a transient disconnect.  The retry loop in
//! [`super::retry::call_with_retry`] invalidates the logical "connected" state
//! on a retryable error so the next attempt goes through the channel's built-in
//! reconnect logic (RISK-10).
...
///
/// Service wrappers for individual RPC methods are implemented in separate
/// modules (tasks 8–16) and call [`DaemonClient::call`] / [`DaemonClient::call_search`].
pub struct DaemonClient {
    // One typed client per service.  Each internally clones the channel, so
    // all share the same underlying connection pool.
...
impl DaemonClient {
    ...
    pub fn new(endpoint: &str) -> Result<Self, ClientError> {
        ...
        Ok(Self {
            ...
            tracking_write: TrackingWriteServiceClient::new(channel),
            connected: false,
        })
    }

    /// Construct a `DaemonClient` from a loaded [`ServerConfig`].
    ///
    /// Uses `config.daemon.grpc_host` and `config.daemon.grpc_port`.
    pub fn from_config(config: &ServerConfig) -> Result<Self, ClientError> {
        let endpoint = format!(
            "http://{}:{}",
            config.daemon.grpc_host, config.daemon.grpc_port
        );
        Self::new(&endpoint)
    }

    /// Construct a `DaemonClient` at the default address (`http://127.0.0.1:50051`).
    ...
    /// mirror the TS `close()` method and provide an explicit signal to
    /// callers that own the client.
    ...
    ///
    /// This is the **primary dispatch helper** for service method wrappers
    /// (tasks 8–16).  It:
...
// ---------------------------------------------------------------------------
// Per-service accessor helpers (used by service-method modules, tasks 8–16)
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    #[tokio::test]
    async fn from_config_uses_daemon_fields() {
        let mut config = ServerConfig::default();
        config.daemon.grpc_host = "192.168.1.1".to_string();
        config.daemon.grpc_port = 9999;
        let client = DaemonClient::from_config(&config);
        assert!(client.is_ok(), "valid config must produce a client");
    }
    ...
    async fn call_search_uses_10s_budget_not_5s() {
        // With a tiny override we can verify call_search resolves with the
        // correct budget label.  A sleeping closure + 1 ms override times out.
        let mut client = DaemonClient::new("http://127.0.0.1:50051").unwrap();
    ...
    async fn call_search_method_name_infers_10s_budget() {
        // Using call() with method_name="search" and no override should resolve
        // timeout via resolve_timeout → 10 s.  Test with instant success.
        let mut client = DaemonClient::new("http://127.0.0.1:50051").unwrap();
