...
async fn semantic_mode_dense_threshold_forwarded_no_sparse_leg() {
    ...
    let _ = run_search_pipeline(&mut daemon, &qdrant, Vec::new(), &opts, None, false, &Default::default()).await;
...
// ---------------------------------------------------------------------------
// parse_args permissive parsing — split to sibling for size compliance
// ---------------------------------------------------------------------------

#[path = "search_tests_score_part2.rs"]
