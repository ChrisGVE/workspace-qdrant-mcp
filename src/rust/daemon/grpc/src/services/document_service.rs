...
//!
//! ## Embedding Strategy (ADR-001)
//!
//! **IMPORTANT**: This service currently uses mock embeddings (hash-based deterministic
//! vectors). This is a placeholder and MUST be replaced with Qdrant-managed FastEmbed.
//!
//! Target configuration (not yet implemented):
//! - Qdrant collections configured with FastEmbed vectorizer
//! - Model: all-MiniLM-L6-v2 (384 dimensions)
//! - Embeddings generated server-side by Qdrant from payload.content
//! - Dense vector field auto-populated by Qdrant
//!
//! See: https://qdrant.tech/documentation/fastembed/
//!
//! ## Multi-Tenant Routing (ADR-001)
//!
//! Routes content to canonical collections based on collection_basename:
//! - `memory` → Canonical 'memory' collection
//! - `projects` → Canonical 'projects' collection with project_id metadata
//! - `libraries` → Canonical 'libraries' collection with library_name metadata
...
/// NOTE (ADR-001): When FastEmbed is enabled, Qdrant manages this automatically
...
impl DocumentServiceImpl {
    ...
    /// Determine the target collection and tenant metadata for multi-tenant routing (ADR-001)
    ///
    /// Canonical routing logic:
    /// - `memory` → Canonical 'memory' collection
    /// - Project content (12-char hex tenant_id) → 'projects' collection with project_id
    /// - Library content (non-hex tenant_id) → 'libraries' collection with library_name
    ///
    /// Returns: (collection_name, tenant_type, tenant_value)
    /// - tenant_type: "project_id" or "library_name"
    ///
    /// NOTE: Uses UNIFIED_PROJECTS_COLLECTION and UNIFIED_LIBRARIES_COLLECTION constants
    /// which should be updated to "projects" and "libraries" per ADR-001.
    fn determine_collection_routing(
        ...
        // Memory collection: canonical 'memory' name (ADR-001)
        if basename == "memory" {
            // TODO (ADR-001): Update to use canonical "memory" collection without tenant suffix
        ...
        if Self::is_project_id(tenant_id) {
            // 12-char hex = project_id → route to projects collection (ADR-001)
            // TODO: Update UNIFIED_PROJECTS_COLLECTION to "projects"
            Ok((
            ...
            // Non-hex = library_name → route to libraries collection (ADR-001)
            // TODO: Update UNIFIED_LIBRARIES_COLLECTION to "libraries"
            Ok((
    ...
    /// DEPRECATED: Generate mock embedding for text
    ///
    /// **WARNING**: This is a PLACEHOLDER implementation that generates deterministic
    /// hash-based vectors. Semantic search WILL NOT work correctly with these embeddings!
    ///
    /// **MIGRATION (ADR-001)**:
    /// Replace with Qdrant-managed FastEmbed configuration:
    /// 1. Configure collection with FastEmbed vectorizer on creation
    /// 2. Remove dense_vector from DocumentPoint
    /// 3. Ensure payload.content contains text for Qdrant to embed
    /// 4. Let Qdrant generate embeddings server-side
    ///
    /// See: https://qdrant.tech/documentation/fastembed/
    ///
    /// Returns: Deterministic mock vector (NOT semantically meaningful)
    #[deprecated(since = "0.4.0", note = "Use Qdrant-managed FastEmbed instead")]
    ...
    /// Collection creation should be updated to configure FastEmbed vectorizer:
    /// - Vector config: Use FastEmbed with all-MiniLM-L6-v2 model
    /// - Indexing: Enable payload indexing for project_id, library_name, tags
    /// - Vectors: Auto-generated from payload.content by Qdrant
    ///
    /// Current implementation creates basic vector collection without FastEmbed.
    async fn ensure_collection_exists(&self, collection_name: &str) -> Result<(), Status> {
        match self.storage_client.collection_exists(collection_name).await {
            ...
            Ok(false) => {
                // TODO (ADR-001): Configure FastEmbed vectorizer on collection creation
                // See: https://qdrant.tech/documentation/fastembed/
                info!("Creating collection '{}' (NOTE: FastEmbed not yet configured)", collection_name);
                self.storage_client
