...
async fn test_operational_state_component_isolation() {
    ...
    set_operational_state(pool, "version", "daemon", "1.0", None)
        .await
        .unwrap();
    set_operational_state(pool, "version", "server", "2.0", None)
        .await
        .unwrap();
    set_operational_state(pool, "version", "cli", "3.0", None)
        .await
        .unwrap();

    assert_eq!(
        get_operational_state(pool, "version", "daemon", None)
            .await
            .unwrap(),
        Some("1.0".to_string())
    );
    assert_eq!(
        get_operational_state(pool, "version", "server", None)
            .await
            .unwrap(),
        Some("2.0".to_string())
    );
    assert_eq!(
        get_operational_state(pool, "version", "cli", None)
            .await
            .unwrap(),
        Some("3.0".to_string())
    );
...
async fn test_operational_state_project_scoped() {
    ...
    // Global entry
    set_operational_state(pool, "status", "daemon", "running", None)
        .await
        .unwrap();
    // Project-scoped entry
    set_operational_state(pool, "status", "daemon", "indexing", Some("proj-1"))
        .await
        .unwrap();

    // Global lookup should NOT return project-scoped value
    let global = get_operational_state(pool, "status", "daemon", None)
        .await
        .unwrap();
    assert_eq!(global, Some("running".to_string()));

    // Project lookup should return project-scoped value
    let proj = get_operational_state(pool, "status", "daemon", Some("proj-1"))
        .await
        .unwrap();
    assert_eq!(proj, Some("indexing".to_string()));

    // Unknown project returns None
    let unknown = get_operational_state(pool, "status", "daemon", Some("proj-999"))
        .await
        .unwrap();
...
async fn test_operational_state_missing_key() {
    ...
    let val = get_operational_state(pool, "nonexistent", "daemon", None)
        .await
        .unwrap();
    assert_eq!(val, None);
}
