...
pub(crate) async fn process_delete_tenant_item(
    ...
    let (points_by_collection, file_count) = collect_tracked_points(pool, &item.tenant_id).await?;
...
async fn delete_tracked_qdrant_points(
    ...
    for (collection, point_ids) in points_by_collection {
        ...
        info!(
            "Step 3: deleted {} points from '{}' (tenant delete)",
            point_ids.len(),
            collection
        );
    }
...
async fn delete_collection_points(
    ...
    info!(
        "Step {}: deleted {} points from '{}' for tenant={}",
        step_num,
        point_ids.len(),
        collection,
        tenant_id
...
async fn sqlite_cascade_delete(tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>, tenant_id: &str) {
    ...
    let cascade_steps: &[(&str, &str, &str)] = &[
        ...
        (
            "6c",
            "DELETE FROM keywords WHERE tenant_id = ?1",
            "keywords",
        ),
        (
            "6d",
            "DELETE FROM keyword_baskets WHERE tenant_id = ?1",
            "keyword_baskets",
        ),
        ("6e", "DELETE FROM tags WHERE tenant_id = ?1", "tags"),
        (
            "6f",
            "DELETE FROM tag_hierarchy_edges WHERE tenant_id = ?1",
            "tag_hierarchy_edges",
        ),
        (
            "6g",
            "DELETE FROM canonical_tags WHERE tenant_id = ?1",
            "canonical_tags",
        ),
        (
            "6h1",
