...
impl LuaExtractor {
    ...
    fn create_parser(&self) -> Result<TreeSitterParser, DaemonError> {
        match &self.language {
            Some(lang) => TreeSitterParser::with_language("lua", lang.clone()),
            None => TreeSitterParser::new("lua"),
        }
    }
    ...
    fn extract_function(&self, node: &Node, source: &str, file_path: &str) -> SemanticChunk {
        // For function_declaration: `function name(...) ... end`
        // For local_function: `local function name(...) ... end`
        let name = find_child_by_kind(node, "identifier")
            .or_else(|| find_child_by_kind(node, "dot_index_expression"))
            .or_else(|| find_child_by_kind(node, "method_index_expression"))
        ...
        // Detect method vs function (M:method or M.method pattern)
        let is_method = name.contains(':') || name.contains('.');
        let chunk_type = if is_method {
            ChunkType::Method
        } else {
            ChunkType::Function
        };
        ...
        let mut chunk = SemanticChunk::new(
            chunk_type, name, content, start_line, end_line, "lua", file_path,
        )
        .with_calls(calls);
    ...
    fn extract_doc_comment(&self, node: &Node, source: &str) -> Option<String> {
        ...
        while let Some(sibling) = prev {
            if sibling.kind() == "comment" {
                ...
                // Lua doc comments typically start with ---
...
impl ChunkExtractor for LuaExtractor {
    fn extract_chunks(
        &self,
        source: &str,
        file_path: &Path,
...
mod tests {
    ...
    fn test_extract_function() {
        let Some(lang) = get_language("lua") else {
            return;
        };
        let source = r#"
--- Greet a person
...
"#;
        ...
        let extractor = LuaExtractor::with_language(lang);
...
"#;
        ...
        let fn_chunk = chunks.iter().find(|c| c.chunk_type == ChunkType::Function);
        assert!(fn_chunk.is_some());
        assert_eq!(fn_chunk.unwrap().symbol_name, "helper");
    }
}
