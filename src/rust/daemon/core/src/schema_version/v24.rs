...
impl Migration for V24Migration {
    async fn up(&self, pool: &SqlitePool) -> Result<(), SchemaError> {
        ...
        use crate::dependency_grouper::{
            CREATE_PROJECT_DEPENDENCIES_INDEXES_SQL, CREATE_PROJECT_DEPENDENCIES_SQL,
        };
        use crate::project_groups_schema::{
            CREATE_PROJECT_GROUPS_INDEXES_SQL, CREATE_PROJECT_GROUPS_SQL,
        };

        sqlx::query(CREATE_PROJECT_GROUPS_SQL).execute(pool).await?;
    ...
    fn description(&self) -> &'static str {
        "Create project_groups and project_dependencies tables"
    }
}
