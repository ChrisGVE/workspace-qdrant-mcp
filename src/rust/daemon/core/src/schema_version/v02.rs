...
impl Migration for V02Migration {
    async fn up(&self, pool: &SqlitePool) -> Result<(), SchemaError> {
        ...
        use crate::tracked_files_schema::{
            CREATE_QDRANT_CHUNKS_INDEXES_SQL, CREATE_QDRANT_CHUNKS_SQL,
            CREATE_TRACKED_FILES_INDEXES_SQL, CREATE_TRACKED_FILES_SQL,
        };
        ...
        sqlx::query(CREATE_TRACKED_FILES_SQL).execute(pool).await?;
        ...
        sqlx::query(CREATE_QDRANT_CHUNKS_SQL).execute(pool).await?;
    ...
    fn description(&self) -> &'static str {
        "Create tracked_files and qdrant_chunks tables"
    }
}
