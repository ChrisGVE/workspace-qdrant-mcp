...
async fn attach_context_lines(
    ...
    for (file_id, match_indices) in &file_matches {
        // Compute the range of line numbers needed for this file
...
ORDER BY line_number"#,
        ...
        // Build a line_number → content map for fast lookup
        let line_map: HashMap<i64, String> = rows
            .iter()
            .map(|row| (row.get::<i64, _>("line_number"), row.get::<String, _>("content")))
            .collect();

        // Distribute context to each match in this file
        for &idx in match_indices {
            let match_line = matches[idx].line_number;

            // Collect context_before (N lines before the match)
            ...
            // Collect context_after (N lines after the match)
...
mod tests {
    ...
    // ── Context line retrieval tests (Task 56) ──
    ...
    async fn test_context_lines_across_files() {
        ...
        insert_file_content(
            &db, 1,
            &["// before", "fn target() {}", "// after"],
            "proj1", Some("main"), "src/a.rs",
        ).await;
        insert_file_content(
            &db, 2,
            &["// pre", "fn target() {}", "// post"],
            "proj1", Some("main"), "src/b.rs",
        ).await;
    ...
    async fn test_context_lines_large_context() {
        ...
        insert_file_content(
            ...
            "proj1", Some("main"), "src/large.rs",
        ).await;

        // Request 10 context lines but file only has 2 before and 2 after
        let results = search_exact(
            &db,
            "target",
            &SearchOptions {
