//! NetworkDiscovery implementation — socket lifecycle, announce, discover, health
...
impl NetworkDiscovery {
    ...
    pub fn new(
        ...
        if !multicast_ip.is_multicast() {
            return Err(NetworkError::InvalidMulticastAddress(format!(
        ...
        Ok(Self {
            multicast_addr,
            socket: Arc::new(tokio::sync::RwLock::new(None)),
            discovered_services: Arc::new(tokio::sync::RwLock::new(HashMap::new())),
            event_sender,
            auth_token,
            cache_timeout: Duration::from_secs(300), // 5 minutes
            local_services: Arc::new(tokio::sync::RwLock::new(HashMap::new())),
        })
    }
    ...
    pub async fn discover_services(
        ...
        while start_time.elapsed().unwrap_or(Duration::MAX) < discover_timeout {
            match timeout(Duration::from_millis(100), event_receiver.recv()).await {
                Ok(Ok(DiscoveryEvent::ServiceDiscovered {
                    service_name,
                    service_info,
                })) => {
                ...
                Ok(Ok(DiscoveryEvent::ServiceUpdated {
                    service_name,
                    service_info,
                })) => {
                    if service_names.is_empty() || service_names.contains(&service_name) {
    ...
    pub(super) async fn _process_message(
        &self,
        message: DiscoveryMessage,
        sender_addr: SocketAddr,
    ) -> Result<(), NetworkError> {
        debug!(
            "Processing {} message from {} for service {}",
            serde_json::to_string(&message.message_type).unwrap_or_default(),
            sender_addr,
            message.service_name
        );
    ...
    pub(super) async fn _cache_discovered_service(
        &self,
        service_name: &str,
        service_info: ServiceInfo,
    ) {
    ...
    pub(super) async fn _remove_cached_service(&self, service_name: &str) {
        let mut services_guard = self.discovered_services.write().await;
        services_guard.remove(service_name);
    }

    /// Start cache cleanup task to remove expired entries
    pub(super) async fn start_cache_cleanup(&self) {
