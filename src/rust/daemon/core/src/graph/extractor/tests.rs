...
fn test_calls_skip_turbofish_artifacts() {
    ...
    let result = extract_edges_from_text_chunks(&[chunk], "t1", "src/lib.rs", None);
...
fn test_is_valid_symbol_name_rejects_generic_fragments() {
    ...
    // All-underscore segments are placeholders, not symbols.
    assert!(!is_valid_symbol_name("_"));
    assert!(!is_valid_symbol_name("__"));
    assert!(!is_valid_symbol_name("foo::_")); // wildcard segment in a path
    assert!(is_valid_symbol_name("_x")); // but a leading underscore + char is real
}
