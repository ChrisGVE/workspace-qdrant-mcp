...
proptest! {
                ...
                .map(|_| {
                    thread::spawn(|| {
                        ...
                        if fd >= 0 {
                            ...
                            // dup failure is acceptable (fd limit)
                            true
                        }
                    })
                })
    ...
    fn test_buffer_bounds_checking(buf_size in 1usize..2048, access_offset in 0usize..4096) {
        // Test that bounds checking prevents buffer overflows
        let buffer: Vec<u8> = vec![0xAA; buf_size];
        let ptr = buffer.as_ptr();

        if access_offset < buf_size {
            // Within bounds: safe to read
            let value = unsafe { *ptr.add(access_offset) };
            prop_assert_eq!(value, 0xAA);
        }
        // Out of bounds: we do NOT dereference, verifying the pattern
        // that all unsafe code should check bounds before access
        prop_assert!(buf_size > 0); // Buffer is always non-empty
    }
    ...
    fn test_memory_allocation_tracking(
        ...
        // Test that memory tracking correctly tracks allocations/deallocations
        ...
        // Double-free should return None
    ...
    fn test_null_pointer_checks(is_null in proptest::bool::ANY) {
        // Test that null pointer checks work correctly
        if is_null {
            let ptr: *const u8 = std::ptr::null();
            prop_assert!(ptr.is_null());
            // We should NEVER dereference a null pointer
        } else {
            let value: u8 = 42;
            let ptr: *const u8 = &value;
            prop_assert!(!ptr.is_null());
            // Safe to dereference non-null pointer to valid data
