...
impl ColumnarBuilder {
    ...
    fn compute_max_key_width(&self, entries: &[ColumnarEntry], depth: usize) -> usize {
        let base_indent = Gutter::WIDTH + depth * self.indent;
        ...
        for entry in entries {
            match entry {
                ...
                ColumnarEntry::Nested { key, entries } => {
                    ...
                    max_w = max_w.max(w);
                    let nested_w = self.compute_max_key_width(entries, depth + 1);
        ...
        for entry in entries {
            match entry {
                ...
                ColumnarEntry::Nested { entries, .. } => {
                    let nested_w = self.compute_content_width(entries, depth + 1, max_key_width);
                    max_w = max_w.max(nested_w);
                }
                ColumnarEntry::Raw { text, .. } => {
                    let w =
                        Gutter::WIDTH + depth * self.indent + UnicodeWidthStr::width(text.as_str());
                    max_w = max_w.max(w);
                }
                _ => {}
            }
