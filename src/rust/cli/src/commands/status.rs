...
EXAMPLES:
    wqm status                  Quick overview of daemon status
    wqm status --queue          Include queue depth in status
    wqm status --watch          Include file watcher status
    wqm status live             Real-time updating dashboard
    wqm status live -i 5        Update every 5 seconds
    wqm status queue -v         Detailed queue with pending items
    wqm status errors -n 20     Show last 20 errors
    wqm status messages clear   Clear all system messages

DASHBOARD (live mode):
    Press Ctrl+C to exit live mode. The dashboard shows:
...
FLAGS VS SUBCOMMANDS:
    Use flags (--queue, --watch) to include info in default status.
    Use subcommands (queue, watch) for dedicated detailed views.";
...
pub struct StatusArgs {
    ...
    /// Include queue depth in status overview
    #[arg(long, help = "Add queue information to default status")]
    queue: bool,

    /// Include file watcher status in overview
    #[arg(long, help = "Add file watcher info to default status")]
    watch: bool,

    /// Include performance metrics in overview
    #[arg(long, help = "Add CPU/memory metrics to default status")]
    performance: bool,
}
...
#[derive(Subcommand)]
enum StatusCommand {
    /// Show historical metrics over time
    #[command(long_about = "Display historical system metrics showing trends over time. \
        Useful for identifying performance patterns or issues.")]
    History {
        /// Time range: 1h (1 hour), 24h (1 day), 7d (1 week)
        #[arg(short, long, default_value = "1h", help = "Time range: 1h, 24h, or 7d")]
        range: String,
    },

    /// Show detailed ingestion queue status
    #[command(long_about = "Display the ingestion queue with pending items, \
        processing statistics, and any failed entries.")]
    Queue {
        /// List individual pending items
        #[arg(short, long, help = "Show individual queue entries")]
        verbose: bool,
    },

    /// Show file watcher status for all watched paths
    #[command(long_about = "Display status of all active file watchers including:\n\
        • Watched paths and patterns\n\
        • Events detected\n\
        • Any watcher errors")]
    Watch,

    /// Show system performance metrics
    #[command(long_about = "Display detailed performance metrics:\n\
    ...
    /// Real-time updating status dashboard
    #[command(long_about = "Launch a live-updating dashboard that refreshes automatically. \
        Shows daemon status, queue depth, active watchers, and recent activity. \
        Press Ctrl+C to exit.")]
    Live {
        /// Refresh interval in seconds (default: 2)
        #[arg(short, long, default_value = "2", help = "Seconds between updates")]
        interval: u64,
    },

    /// View and manage system messages
    #[command(long_about = "View system messages (notifications, warnings) or clear them. \
        Use 'messages list' to view or 'messages clear' to remove all.")]
    Messages {
        #[command(subcommand)]
    ...
    #[command(long_about = "Display recent errors from the daemon with timestamps, \
        error types, and stack traces (if available).")]
    Errors {
        /// Number of errors to display
        #[arg(short = 'n', long, default_value = "10", help = "Maximum errors to show")]
        limit: usize,
    },

    /// Show detailed system health status
    #[command(long_about = "Display health status for all system components with \
        pass/fail indicators and diagnostic information.")]
    Health,
}
...
#[derive(Subcommand)]
enum MessageAction {
    /// List all pending system messages
    List,
