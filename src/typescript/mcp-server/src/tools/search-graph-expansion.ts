...
function collectCandidates(results: SearchResult[]): ExpandCandidate[] {
  const candidates: ExpandCandidate[] = [];
  for (const r of results) {
    const symbolName = r.metadata['chunk_symbol_name'] as string | undefined;
    const chunkType = r.metadata['chunk_chunk_type'] as string | undefined;
    const tenantId = r.metadata['tenant_id'] as string | undefined;
    const filePath = (r.metadata['relative_path'] as string) ?? (r.metadata['file_path'] as string);
...
function graphProximityScore(hopDistance: number): number {
  if (hopDistance <= 0) return 1.0;
  if (hopDistance === 1) return 0.8;
  return 0.5;
}

function nodeToSearchResult(
  node: TraversalNodeProto,
  collection: string,
  tenantId: string,
  proximity: number
): SearchResult {
 ...
 */
export async function expandAndFuseWithGraph(
    ...
    try {
      const response = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('timeout')), GRAPH_EXPANSION_TIMEOUT_MS)
        ),
      ]);

      if (!response.nodes) return [];

      const newNodes: SearchResult[] = [];
      for (const node of response.nodes.slice(0, MAX_EXPANDED_PER_RESULT)) {
        if (node.node_id === candidate.nodeId) continue;
        ...
        if (totalExpanded >= MAX_EXPANDED_TOTAL) break;
