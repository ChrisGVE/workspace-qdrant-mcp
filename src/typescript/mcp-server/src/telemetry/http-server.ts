 ...
 */
export function formatListenError(err: Error & { code?: string }, port: number): string {
...
export function startMetricsServer(port?: number): Server {
  ...
  if (!Number.isInteger(rawPort) || rawPort < 1 || rawPort > 65535) {
    throw new Error(
      `[wqm-metrics] Invalid MCP_METRICS_PORT value "${String(process.env['MCP_METRICS_PORT'] ?? rawPort)}". Must be an integer in [1, 65535].`
    );
  }
  ...
  if (!loopback && !token) {
    process.stderr.write(
      `[wqm-metrics] WARNING: metrics server bound to non-loopback host "${host}" without MCP_METRICS_TOKEN set. All /metrics requests will be rejected (401). Set MCP_METRICS_TOKEN to allow scraping.\n`
    );
  }
  ...
  server.listen(listenPort, host, () => {
    process.stderr.write(
      `[wqm-metrics] HTTP metrics server listening on ${host}:${listenPort}/metrics\n`
    );
  });
