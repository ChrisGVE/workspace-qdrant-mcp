...
/** prom-client getMetricsAsJSON() item shape (subset we use). */
interface PromMetricFamily {
  name: string;
  help: string;
  type: string;
...
function labelsToAttributes(labels: Record<string, string>): OtlpKv[] {
 ...
 */
function counterToOtlp(family: PromMetricFamily): Record<string, unknown> {
 ...
 */
function gaugeToOtlp(family: PromMetricFamily): Record<string, unknown> {
  const nowNs = msToNs(Date.now());
...
function familyToOtlp(family: PromMetricFamily): Record<string, unknown> | null {
  switch (family.type) {
    case 'counter':
...
function buildOtlpPayload(families: PromMetricFamily[]): Record<string, unknown> {
 ...
 */
export async function pushMetricsToOTLP(): Promise<void> {
  try {
    const endpoint = resolveEndpoint();
    const extraHeaders = resolveHeaders();
    const url = `${endpoint}/v1/metrics`;

    const families = (await register.getMetricsAsJSON()) as unknown as PromMetricFamily[];
