...
// ── SQL helpers ───────────────────────────────────────────────────────────
function buildTenantClause(tenantId) {
    return tenantId ? 'AND t.tenant_id = ?' : '';
}
function buildTenantParams(base, tenantId) {
 ...
 */
export function getMatchingTags(db, query, collection, tenantId) {
    if (!db)
        return [];
    try {
        ...
        const rows = db
            .prepare(`SELECT DISTINCT t.tag_id, t.tag, t.score
        FROM tags t
        WHERE t.collection = ?
 ...
 */
export function getKeywordBasketsForTags(db, tagIds) {
    if (!db || tagIds.length === 0)
        return [];
    try {
        const placeholders = tagIds.map(() => '?').join(',');
        const rows = db
            .prepare(`SELECT kb.tag_id, kb.keywords_json
 ...
 */
export function listTags(db, collection, tenantId, limit = 50) {
    if (!db)
        return [];
    try {
        const tenantClause = buildTenantClause(tenantId);
 ...
 */
export function getTagHierarchy(db, collection, tenantId) {
    if (!db)
        return [];
    try {
        ...
        const params = buildTenantParams([collection], tenantId);
        const rows = db
            .prepare(`SELECT ct.canonical_name,
               ct.level,
