...
function buildRotatingFileLogger(options) {
 ...
 */
export function logToolCall(tool, durationMs, success, context) {
    const log = createSessionLogger();
    log.info({
        tool,
 ...
 */
export function logSessionEvent(event, context) {
    const log = createSessionLogger();
 ...
 */
export function logDaemonStatus(connected, context) {
    const log = createSessionLogger();
    if (connected) {
