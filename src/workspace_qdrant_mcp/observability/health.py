...
class HealthChecker:
    ...
    def __init__(self):
        ...
        if (
            ...
            logger.info(
                "HealthChecker initialized", checks_registered=len(self.health_checks)
            )
    ...
    def _initialize_standard_checks(self):
        ...
        logger.debug(
            "Standard health checks registered", checks=list(self.health_checks.keys())
        )
    ...
    async def run_check(self, name: str) -> ComponentHealth:
        ...
        if not check.enabled:
            return ComponentHealth(
                name=name, status=HealthStatus.UNKNOWN, message="Health check disabled"
            )
        ...
        try:
            ...
            metrics_instance.increment_counter(
                "health_checks_total", check=name, status=status.value
            )
            metrics_instance.record_histogram(
                "health_check_duration_seconds", response_time, check=name
            )
            ...
            metrics_instance.increment_counter(
                "health_checks_total", check=name, status="timeout"
            )
            ...
            metrics_instance.increment_counter(
                "health_checks_total", check=name, status="error"
            )
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        metrics_instance.set_gauge(
            "system_health_status", 1 if overall_status == HealthStatus.HEALTHY else 0
        )
    ...
    async def _get_system_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "system_disk": psutil.disk_usage("/")._asdict()
                if Path("/").exists()
                else None,
            }
        except Exception as e:
        ...
        try:
            ...
            return {
                ...
                "details": {
                    ...
                    "embedding_model": status.get("embedding_info", {}).get(
                        "model_name"
                    ),
                },
            }
    ...
    async def _check_file_watchers(self) -> Dict[str, Any]:
        ...
        try:
            ...
            watch_list = await watch_tools_manager.list_watched_folders(
                active_only=False, include_stats=True
            )
    ...
    async def _check_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status = (
                HealthStatus.DEGRADED if results["warnings"] else HealthStatus.HEALTHY
            )
    ...
    async def _background_monitor(self):
        ...
        try:
            while self._enabled:
                ...
                try:
                    ...
                    if health_status["status"] != "healthy":
                        ...
                        logger.debug(
                            "Health check completed", status=health_status["status"]
                        )
            ...
            logger.error(
                "Background health monitoring failed", error=str(e), exc_info=True
            )
