...
class OperationMonitor:
    ...
    def __enter__(self):
        ...
        with LogContext(operation=self.operation_name, operation_id=self.operation_id, **self.context):
            logger.info(f"Operation started: {self.operation_name}", 
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.start_time is None:
            logger.error("Operation monitor not properly started", 
        ...
        if self.timeout_warning and duration > self.timeout_warning:
            performance_issues.append(f"exceeded timeout warning ({self.timeout_warning}s)")
        
        if self.slow_threshold and duration > self.slow_threshold:
            performance_issues.append(f"exceeded slow threshold ({self.slow_threshold}s)")
        
        # Log completion
        with LogContext(**log_context):
            if not success:
                if self.critical:
                    logger.critical(f"Critical operation failed: {self.operation_name}",
                                   exc_info=True)
                else:
                    logger.error(f"Operation failed: {self.operation_name}",
                               exc_info=True)
            elif performance_issues:
                logger.warning(f"Operation completed with issues: {self.operation_name}",
        ...
        if performance_issues:
            metrics_instance.increment_counter("slow_operations_total",
                                             operation=self.operation_name,
                                             **self.context)
        
        if self.critical and not success:
            metrics_instance.increment_counter("critical_failures_total",
                                             operation=self.operation_name,
                                             **self.context)
...
async def async_operation_monitor(
    ...
    **context
    ...
    monitor = OperationMonitor(operation_name, critical, timeout_warning, slow_threshold, **context)
    ...
    **default_context
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args, **kwargs):
            ...
            with OperationMonitor(op_name, critical, timeout_warning, slow_threshold, **context):
                try:
    ...
    **default_context
    ...
    def decorator(func: AF) -> AF:
        ...
        async def wrapper(*args, **kwargs):
            ...
            # Monitor the operation
            async with async_operation_monitor(op_name, critical, timeout_warning, slow_threshold, **context):
                try:
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    if duration > critical_threshold:
                        logger.critical("Performance critical: function exceeded critical threshold",
                                      threshold_seconds=critical_threshold, **context)
                        metrics_instance.increment_counter("performance_critical_total", 
                                                         function=func.__name__)
                    elif duration > slow_threshold:
                        logger.warning("Performance warning: function is slow",
                                     threshold_seconds=slow_threshold, **context)
                        metrics_instance.increment_counter("performance_slow_total",
                                                         function=func.__name__)
                    
                    if memory_threshold_mb and memory_delta > memory_threshold_mb:
                        logger.warning("Memory usage warning: function used significant memory",
                                     threshold_mb=memory_threshold_mb, **context)
                        metrics_instance.increment_counter("memory_heavy_operations_total",
                                                         function=func.__name__)
                    
                    # Record performance metrics
                    metrics_instance.record_histogram("function_duration_seconds", duration,
                                                     function=func.__name__)
                    metrics_instance.record_histogram("function_memory_delta_mb", memory_delta,
                                                     function=func.__name__)
                    ...
                    duration = time.perf_counter() - start_time
                    logger.error("Performance monitoring: function failed",
                               function=func.__name__,
                               duration_seconds=duration,
                               error=str(e),
                               exc_info=True)
...
class BatchOperationMonitor:
    ...
    def __enter__(self):
        ...
        logger.info(f"Batch operation started: {self.operation_name}",
                   operation_id=self.operation_id,
                   total_items=self.total_items,
                   **self.context)
        
        metrics_instance.set_gauge("batch_operation_total_items", 
                                 self.total_items,
                                 operation=self.operation_name,
                                 **self.context)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        duration = time.perf_counter() - self.start_time
        success_rate = (self.processed_items - self.failed_items) / max(1, self.processed_items)
        
        logger.info(f"Batch operation completed: {self.operation_name}",
                   operation_id=self.operation_id,
                   duration_seconds=duration,
        ...
        # Record metrics
        metrics_instance.record_histogram("batch_operation_duration_seconds", duration,
                                         operation=self.operation_name, **self.context)
        metrics_instance.record_histogram("batch_operation_success_rate", success_rate,
                                         operation=self.operation_name, **self.context)
        metrics_instance.set_gauge("batch_operation_total_items", 0,
                                 operation=self.operation_name, **self.context)
    ...
    def record_item_processed(self, success: bool = True):
        ...
        metrics_instance.set_gauge("batch_operation_processed_items", 
        ...
        if self.processed_items % max(1, self.total_items // 10) == 0:
            ...
            logger.debug(f"Batch progress: {self.operation_name}",
                        operation_id=self.operation_id,
...
def monitor_batch_operation(operation_name: str, total_items: int, **context) -> BatchOperationMonitor:
