...
class WatchToolsManager:
    ...
    async def add_watch_folder(
        ...
        try:
            ...
            if not watch_id:
                watch_id = f"watch_{uuid.uuid4().hex[:8]}_{int(datetime.now().timestamp())}"
    ...
    async def list_watched_folders(
        ...
        try:
            ...
            summary = {
                ...
                "disabled_watches": len([w for w in all_configs if w.status == "disabled"]),
            }
            
            if include_stats:
                summary.update({
                    "total_files_processed": sum(w.files_processed for w in all_configs),
    ...
    async def configure_watch_settings(
        ...
        try:
            ...
            if patterns is not None:
                changes_made["patterns"] = {"old": existing_config.patterns, "new": patterns}
                existing_config.patterns = patterns
            
            if ignore_patterns is not None:
                changes_made["ignore_patterns"] = {"old": existing_config.ignore_patterns, "new": ignore_patterns}
                existing_config.ignore_patterns = ignore_patterns
            
            if auto_ingest is not None:
                changes_made["auto_ingest"] = {"old": existing_config.auto_ingest, "new": auto_ingest}
                existing_config.auto_ingest = auto_ingest
            
            if recursive is not None:
                changes_made["recursive"] = {"old": existing_config.recursive, "new": recursive}
                existing_config.recursive = recursive
            
            if recursive_depth is not None:
                changes_made["recursive_depth"] = {"old": existing_config.recursive_depth, "new": recursive_depth}
                existing_config.recursive_depth = recursive_depth
            
            if debounce_seconds is not None:
                changes_made["debounce_seconds"] = {"old": existing_config.debounce_seconds, "new": debounce_seconds}
                existing_config.debounce_seconds = debounce_seconds
            
            if update_frequency is not None:
                changes_made["update_frequency"] = {"old": existing_config.update_frequency, "new": update_frequency}
