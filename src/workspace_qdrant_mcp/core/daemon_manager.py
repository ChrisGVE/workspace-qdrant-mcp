...
class DaemonInstance:
    ...
    def __init__(self, config: DaemonConfig):
        ...
        logger.info("Created daemon instance", 
    ...
    async def start(self) -> bool:
        ...
        if self.status.state in ["starting", "running"]:
            logger.warning("Daemon already starting/running", project=self.config.project_name)
            return True
        ...
        try:
            ...
            if self.process and self.process.returncode is None:
                try:
                    ...
                    logger.warning("Daemon graceful shutdown timed out, force killing",
                                  project=self.config.project_name)
                    self.process.kill()
                    try:
                        await asyncio.wait_for(self.process.wait(), timeout=5.0)
                    except asyncio.TimeoutError:
                        logger.error("Failed to kill daemon process",
                                    project=self.config.project_name)
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            if self.process.returncode is not None:
                logger.warning("Daemon process has exited", 
                              project=self.config.project_name,
            ...
            try:
                ...
                logger.debug("Health check gRPC connection failed", 
                            project=self.config.project_name,
                            error=str(e))
    ...
    async def _write_config_file(self):
        ...
        logger.debug("Wrote daemon config file", 
                    project=self.config.project_name,
                    config_file=str(self.config_file))
        ...
        if rust_engine_path.exists() and (rust_engine_path / "Cargo.toml").exists():
            ...
            try:
                ...
                result = await asyncio.create_subprocess_exec(
                    "cargo", "build", "--release", "--bin", "workspace-qdrant-grpc",
                ...
                if result.returncode == 0:
                    ...
                    if binary_path.exists():
                        logger.info("Successfully built daemon binary", path=str(binary_path))
                        return binary_path
                else:
                    logger.error("Failed to build daemon binary", 
    ...
    async def _wait_for_startup(self) -> bool:
        ...
        while time.time() - start_time < self.config.startup_timeout:
            if self.process and self.process.returncode is not None:
                logger.error("Daemon process exited during startup", 
                           return_code=self.process.returncode)
                return False
        ...
        logger.error("Daemon startup timeout exceeded", 
        ...
        try:
            while not self.shutdown_event.is_set():
                try:
                        ...
                        if self.config.restart_on_failure:
                            if self.status.restart_count < self.config.max_restart_attempts:
                                ...
                                if not await self.restart():
                                    logger.error("Failed to restart daemon after health check failure",
                                               project=self.config.project_name)
                                    break
                            else:
                    ...
                    logger.error("Error in health monitor loop", 
                               project=self.config.project_name,
                               error=str(e))
            ...
            logger.error("Health monitor loop failed", 
                        project=self.config.project_name,
                        error=str(e))
        ...
        async def read_stream(stream, stream_name):
            try:
                while True:
                    ...
                    if line_str:
                        # Log daemon output
                        log_msg = f"[{self.config.project_name}:{stream_name}] {line_str}"
                        logger.info(log_msg)
                ...
                logger.error("Error monitoring daemon output", 
                           stream=stream_name,
                           error=str(e))
...
class DaemonManager:
    ...
    async def get_or_create_daemon(
        ...
        if daemon_key not in self.daemons:
            logger.info("Creating new daemon instance", 
    ...
    async def start_daemon(
        ...
        daemon = await self.get_or_create_daemon(project_name, project_path, config_overrides)
    ...
    async def get_daemon_status(self, project_name: str, project_path: str) -> Optional[Dict[str, Any]]:
...
async def ensure_daemon_running(
    ...
    daemon = await manager.get_or_create_daemon(project_name, project_path, config_overrides)
...
async def get_daemon_for_project(project_name: str, project_path: str) -> Optional[DaemonInstance]:
