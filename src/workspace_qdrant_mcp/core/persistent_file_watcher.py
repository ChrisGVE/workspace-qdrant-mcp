...
class PersistentFileWatcher(FileWatcher):
    ...
    async def _save_config_state(self, force: bool = False) -> None:
        ...
        try:
            ...
            if force or not self._last_save_time or (
                datetime.fromisoformat(current_time.replace('Z', '+00:00')) - 
                datetime.fromisoformat(self._last_save_time.replace('Z', '+00:00'))
            ).total_seconds() > 30:  # Save at most once every 30 seconds
                
                await self.config_manager.update_watch_config(self.persistent_config)
                self._last_save_time = current_time
                logger.debug(f\"Saved persistent state for watch {self.persistent_config.id}\")
...
class PersistentWatchManager:
    ...
    async def initialize(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f\"Watch manager initialized with {len(self.watchers)} recovered watches\")
            ...
            for config in configs:
                try:
                    ...
                    if validation_issues:
                        logger.warning(f\"Skipping invalid config {config.id}: {', '.join(validation_issues)}\")
                    ...
                    if config.status == \"active\":
                        ...
                        logger.info(f\"Recovered active watch: {config.id} ({config.path})\")
                    else:
                        # Store paused watcher for potential later activation
                        recovery_stats[\"recovered_paused\"] += 1
                        logger.info(f\"Recovered paused watch: {config.id} ({config.path})\")
