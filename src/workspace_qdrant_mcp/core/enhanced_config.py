...
class EnhancedConfig(BaseSettings):
    ...
    def _load_nested_env_vars(self) -> None:
        ...
        env_mappings = [
            ...
            ("WORKSPACE_QDRANT_QDRANT__API_KEY", lambda v: setattr(self.qdrant, 'api_key', v)),
            ("WORKSPACE_QDRANT_QDRANT__TIMEOUT", lambda v: setattr(self.qdrant, 'timeout', int(v))),
            ("WORKSPACE_QDRANT_QDRANT__PREFER_GRPC", lambda v: setattr(self.qdrant, 'prefer_grpc', v.lower() == "true")),
            
            # Embedding configuration
            ("WORKSPACE_QDRANT_EMBEDDING__MODEL", lambda v: setattr(self.embedding, 'model', v)),
            ("WORKSPACE_QDRANT_EMBEDDING__ENABLE_SPARSE_VECTORS", lambda v: setattr(self.embedding, 'enable_sparse_vectors', v.lower() == "true")),
            ("WORKSPACE_QDRANT_EMBEDDING__CHUNK_SIZE", lambda v: setattr(self.embedding, 'chunk_size', int(v))),
            ("WORKSPACE_QDRANT_EMBEDDING__CHUNK_OVERLAP", lambda v: setattr(self.embedding, 'chunk_overlap', int(v))),
            ("WORKSPACE_QDRANT_EMBEDDING__BATCH_SIZE", lambda v: setattr(self.embedding, 'batch_size', int(v))),
            
            # Workspace configuration  
            ("WORKSPACE_QDRANT_WORKSPACE__GITHUB_USER", lambda v: setattr(self.workspace, 'github_user', v)),
            ("WORKSPACE_QDRANT_WORKSPACE__COLLECTION_PREFIX", lambda v: setattr(self.workspace, 'collection_prefix', v)),
            ("WORKSPACE_QDRANT_WORKSPACE__MAX_COLLECTIONS", lambda v: setattr(self.workspace, 'max_collections', int(v))),
        ]
    ...
    def _load_legacy_env_vars(self) -> None:
        ...
        legacy_mappings = [
            ...
            ("ENABLE_SPARSE_VECTORS", lambda v: setattr(self.embedding, 'enable_sparse_vectors', v.lower() == "true")),
            ("CHUNK_SIZE", lambda v: setattr(self.embedding, 'chunk_size', int(v))),
            ("CHUNK_OVERLAP", lambda v: setattr(self.embedding, 'chunk_overlap', int(v))),
    ...
    def validate_config(self) -> List[str]:
        ...
        if self.environment not in valid_environments:
            issues.append(f"Invalid environment '{self.environment}'. Must be one of: {', '.join(valid_environments)}")
