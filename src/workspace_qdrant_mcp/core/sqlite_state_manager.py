...
class SQLiteStateManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.connection.execute("PRAGMA wal_autocheckpoint=1000")  # Auto-checkpoint every 1000 pages
    ...
    async def _setup_schema(self):
        ...
        with self._lock:
            ...
            if not cursor.fetchone():
                ...
                cursor = self.connection.execute("SELECT version FROM schema_version ORDER BY version DESC LIMIT 1")
        ...
        async with self.transaction() as conn:
            ...
            for row in crashed_files:
                ...
                if retry_count < max_retries:
                    ...
                    queue_id = f"recovery_{int(time.time() * 1000)}_{hash(file_path) % 10000}"
                    conn.execute(
                        """
                        ...
                        VALUES (?, ?, ?, ?, ?)
                        """,
                        (queue_id, file_path, collection, ProcessingPriority.HIGH.value, retry_count + 1)
                    )
                    ...
                    recovery_operations += 1
                    logger.warning(f"Marked crashed file as failed (max retries): {file_path}")
        ...
        try:
            with self._lock:
                ...
                if cursor.rowcount > 0:
                    logger.info(f"Cleaned up {cursor.rowcount} old processing history records")
        ...
        try:
            async with self.transaction() as conn:
                ...
                conn.execute(
                    ...
                    """,
                    (
                        file_path, collection, FileProcessingStatus.PROCESSING.value,
                        priority.value, file_size, file_hash, 
                        self._serialize_json(metadata), file_path
                    )
                )
    ...
    async def complete_file_processing(
        ...
        try:
            status = FileProcessingStatus.COMPLETED if success else FileProcessingStatus.FAILED
            ...
            async with self.transaction() as conn:
                ...
                cursor = conn.execute(
                    ...
                    WHERE file_path = ?
                    """,
                    (status.value, error_message, self._serialize_json(metadata), file_path)
                )
                ...
                if row:
                    ...
                    conn.execute(
                        ...
                        """,
                        (
                            file_path, row["collection"], status.value, 
                            processing_time_ms, row["file_size"],
                            error_message, self._serialize_json(metadata)
                        )
                    )
    ...
    async def get_file_processing_status(self, file_path: str) -> Optional[FileProcessingRecord]:
    ...
    async def get_files_by_status(
        ...
        try:
            with self._lock:
                ...
                records = []
                for row in rows:
                    records.append(FileProcessingRecord(
                        file_path=row["file_path"],
                        collection=row["collection"],
                        status=FileProcessingStatus(row["status"]),
                        priority=ProcessingPriority(row["priority"]),
    ...
    async def retry_failed_file(self, file_path: str, max_retries: Optional[int] = None) -> bool:
        ...
        try:
            async with self.transaction() as conn:
                ...
                if current_status not in [FileProcessingStatus.FAILED.value, FileProcessingStatus.SKIPPED.value]:
                    logger.warning(f"File not in failed/skipped state for retry: {file_path} (status: {current_status})")
                    return False
                
                # Update max retries if provided
                new_max_retries = max_retries if max_retries is not None else current_max_retries
                
                if retry_count >= new_max_retries:
                    logger.warning(f"File already at max retries: {file_path} ({retry_count}/{new_max_retries})")
                    return False
        ...
        try:
            async with self.transaction() as conn:
                conn.execute(
                    ...
                    """,
                    (
                        config.watch_id, config.path, config.collection,
                        self._serialize_json(config.patterns),
                        self._serialize_json(config.ignore_patterns),
                        config.auto_ingest, config.recursive, config.recursive_depth,
                        config.debounce_seconds, config.enabled,
                        config.watch_id, self._serialize_json(config.metadata)
                    )
                )
    ...
    async def get_watch_folder_config(self, watch_id: str) -> Optional[WatchFolderConfig]:
        ...
        try:
            with self._lock:
                ...
                return WatchFolderConfig(
                    ...
                    created_at=datetime.fromisoformat(row["created_at"].replace("Z", "+00:00")),
                    updated_at=datetime.fromisoformat(row["updated_at"].replace("Z", "+00:00")),
                    last_scan=datetime.fromisoformat(row["last_scan"].replace("Z", "+00:00")) if row["last_scan"] else None,
                    metadata=self._deserialize_json(row["metadata"])
                )
    ...
    async def get_all_watch_folder_configs(self, enabled_only: bool = True) -> List[WatchFolderConfig]:
        ...
        try:
            ...
            async with self.transaction() as conn:
                ...
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                    """,
                    (file_path, collection, FileProcessingStatus.PENDING.value, priority.value)
                )
                ...
                conn.execute(
                    ...
                    """,
                    (
                        queue_id, file_path, collection, priority.value,
                        scheduled_at.isoformat(), self._serialize_json(metadata)
                    )
                )
                
            logger.debug(f"Added to processing queue: {file_path} (queue_id: {queue_id})")
            return queue_id
    ...
    async def remove_from_processing_queue(self, queue_id: str) -> bool:
        ...
        try:
            async with self.transaction() as conn:
                cursor = conn.execute(
                    "DELETE FROM processing_queue WHERE queue_id = ?",
                    (queue_id,)
                )
        ...
        try:
            ...
            async with self.transaction() as conn:
                cursor = conn.execute(
                    ...
                    """,
                    (new_scheduled_at.isoformat(), max_attempts, queue_id, max_attempts)
                )
    ...
    async def set_system_state(self, key: str, value: Any) -> bool:
        ...
        try:
            serialized_value = self._serialize_json(value) if not isinstance(value, str) else value
        ...
        try:
            ...
            with self._lock:
                ...
                return {
                    ...
                    "success_rate": (
                        (history_stats["successful"] or 0) / max(1, history_stats["total_processed"] or 1)
                    ),
            ...
            return {
                ...
                "queue_stats": {"total": 0, "low": 0, "normal": 0, "high": 0, "urgent": 0}
            }
