...
- PROJECT: "{project}-{suffix}" format (user-created, project-scoped)
...
class CollectionType(Enum):
    ...
    PROJECT = "project"     # {project}-{suffix} format
...
class CollectionTypeClassifier:
    ...
    def classify_collection_type(self, collection_name: str) -> CollectionType:
        ...
        Examples:
            ...
            >>> classifier.classify_collection_type("project-documents")
        ...
        # Check for library collections (_ prefix, but not __)
        ...
        # Check for project collections (contains dash)
    ...
    def is_project_collection(self, collection_name: str) -> bool:
        ...
        Project collections use the {project}-{suffix} format and are
        user-created, project-scoped collections.
    ...
    def is_memory_collection(self, collection_name: str) -> bool:
        ...
        Memory collections can be either:
        - System memory: __{memory_name}
        ...
        """
        return (self._system_memory_pattern.match(collection_name) is not None or
                self._project_memory_pattern.match(collection_name) is not None)
    
    def get_display_name(self, collection_name: str) -> str:
        """
        Get the display name for a collection (removes prefixes for system collections).
        
        For system and library collections, this removes the prefix to create
        ...
        Examples:
            ...
            'library_docs'
            >>> classifier.get_display_name("project-documents")
            'project-documents'
        """
    ...
    def get_collection_info(self, collection_name: str) -> CollectionInfo:
        ...
        if collection_type == CollectionType.SYSTEM:
            ...
            is_searchable = True   # User collections are searchable
            is_readonly = False    # User-writable
        ...
        # Extract project name and suffix for project collections
        project_name = None
        suffix = None
        if collection_type == CollectionType.PROJECT and '-' in collection_name:
    ...
    def extract_project_components(self, collection_name: str) -> Optional[Tuple[str, str]]:
        """
        Extract project name and suffix from project collection names.
        
        Args:
            collection_name: The collection name to parse
            
        Returns:
            Optional[Tuple[str, str]]: Tuple of (project_name, suffix) if project collection,
                                      None otherwise
                                      
        Examples:
            >>> classifier = CollectionTypeClassifier()
            >>> classifier.extract_project_components("my_project-documents")
            ('my_project', 'documents')
            >>> classifier.extract_project_components("__system_config")
            None
        """
        if self.is_project_collection(collection_name) and '-' in collection_name:
            parts = collection_name.split('-', 1)
...
def get_collections_by_type(collections: List[str], 
    ...
    Examples:
        >>> collections = ["__system", "_lib", "project-docs", "algorithms"]
...
def validate_collection_name_with_type(collection_name: str, 
    ...
    Examples:
        ...
        True
        >>> validate_collection_name_with_type("project-docs", CollectionType.SYSTEM)
        False
    """
    classifier = CollectionTypeClassifier()
...
def build_collection_name_for_type(base_name: str, 
    ...
    Args:
        ...
        project_name: Project name (required for project collections)
    ...
    Examples:
        ...
        '__user_prefs'
        >>> build_collection_name_for_type("documents", CollectionType.PROJECT, "myproject")
        'myproject-documents'
    """
    if collection_type == CollectionType.SYSTEM:
        ...
        if not project_name:
            raise ValueError("project_name is required for project collections")
        return collection_naming.build_project_collection_name(project_name, base_name)
    elif collection_type == CollectionType.GLOBAL:
