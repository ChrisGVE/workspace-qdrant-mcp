...
class WatchConfigSchema(BaseModel):
    ...
    recursive_depth: int = Field(
        default=-1, 
        ge=-1, 
        description="Maximum recursive depth (-1 for unlimited)"
    )
...
class PersistentWatchConfigManager:
    ...
    async def add_watch_config(self, watch_config: WatchConfigurationPersistent) -> bool:
    ...
    async def update_watch_config(self, watch_config: WatchConfigurationPersistent) -> bool:
    ...
    async def list_watch_configs(self, active_only: bool = False) -> List[WatchConfigurationPersistent]:
        ...
        try:
            ...
            for config_data in config.watches:
                try:
                    watch_config = WatchConfigurationPersistent.from_dict(config_data.dict())
    ...
    async def get_watch_config(self, watch_id: str) -> Optional[WatchConfigurationPersistent]:
