...
from .parsers import DocumentParser, ParsedDocument, TextParser, MarkdownParser, PDFParser
...
class DocumentIngestionEngine:
    ...
    def __init__(
        ...
        logger.info(f"Initialized DocumentIngestionEngine with {len(self.parsers)} parsers")
    ...
    async def process_directory(
        ...
        try:
            ...
            # Validate collection exists (if not dry run)
            if not dry_run:
                ...
                if collection not in available_collections:
                    return IngestionResult(
                        ...
                        stats=stats,
                        collection=collection,
                        message=f"Collection '{collection}' not found. Available: {available_collections}",
                        dry_run=dry_run
                    )
    ...
    async def _process_single_file(
        ...
        async with self.semaphore:
            try:
                ...
                stats.total_words += len(parsed_doc.content.split()) if parsed_doc.content else 0
                ...
                if not dry_run:
                    ...
                    estimated_chunks = max(1, len(parsed_doc.content) // self.chunk_size)
