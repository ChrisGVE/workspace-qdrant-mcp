...
class PptxParser(DocumentParser):
    ...
    async def parse(self, file_path: str | Path, **options: Any) -> ParsedDocument:
        ...
        try:
            ...
            text_content = await self._extract_text_content(
                prs, include_speaker_notes, include_slide_numbers, 
                slide_separator, extract_table_content, extract_chart_titles,
                include_hidden_slides
            )
            
            # Count slides (including hidden if requested)
            visible_slides = sum(1 for slide in prs.slides if not getattr(slide, 'slide_id', None) or not slide.slide_layout.master.slide_master._element.get('visibility') == '0')
            ...
            logger.info(f"Successfully parsed PPTX: {file_path.name} "
    ...
    async def _extract_metadata(self, prs: "Presentation") -> dict[str, str | int | float | bool]:
        ...
        metadata['slide_aspect_ratio'] = round(slide_width / slide_height, 2) if slide_height > 0 else 0
        ...
        for shape in slide.shapes:
            try:
                ...
                if hasattr(shape, 'text_frame') and shape.text_frame:
                    shape_text = await self._extract_text_frame_content(shape.text_frame)
                    if shape_text:
                ...
                # Handle grouped shapes
                    ...
                    group_text = await self._extract_group_content(shape, extract_table_content)
                    if group_text:
            ...
            if hasattr(shape, 'chart') and shape.chart:
                ...
                if hasattr(chart, 'chart_title') and chart.chart_title:
                    if hasattr(chart.chart_title, 'text_frame') and chart.chart_title.text_frame:
                        title_text = await self._extract_text_frame_content(chart.chart_title.text_frame)
                        if title_text:
    ...
    async def _extract_group_content(self, group_shape, extract_table_content: bool) -> str:
    ...
    async def _extract_speaker_notes(self, slide: "Slide") -> str:
        ...
        try:
            ...
            notes_text = await self._extract_text_frame_content(notes_slide.notes_text_frame)
