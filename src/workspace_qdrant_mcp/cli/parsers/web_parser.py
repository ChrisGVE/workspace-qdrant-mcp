...
class WebParser(DocumentParser):
    ...
    async def parse(self, file_path: Union[str, Path], **options: Any) -> ParsedDocument:
        """
    ...
    async def _process_crawl_results(
        ...
        **options
        ...
        if combined_metadata['security_warnings']:
            parsing_info['security_warnings_count'] = len(combined_metadata['security_warnings'])
            parsing_info['security_status'] = 'warnings_found'
        else:
            parsing_info['security_status'] = 'clean'
        
        logger.info(f"Successfully processed {len(successful_results)} web pages from {original_url}")
