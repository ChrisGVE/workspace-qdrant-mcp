...
class SecurityScanner:
    ...
    def __init__(self):
        ...
        self.malicious_extensions = {
            ...
            '.dll', '.msi', '.jar', '.js', '.vbs', '.ps1'
        }
    
    async def scan_content(self, content: str, content_type: str) -> Tuple[bool, List[str]]:
        ...
        critical_warnings = [w for w in warnings if any(keyword in w.lower() 
                           for keyword in ['script', 'javascript', 'binary'])]
...
class SecureWebCrawler:
    ...
    async def _validate_url(self, url: str, result: CrawlResult) -> bool:
        ...
        try:
            ...
            if len(url) > self.config.max_url_length:
                result.error = f"URL too long: {len(url)} > {self.config.max_url_length}"
                return False
            ...
            # Check allowlist
            if self.config.domain_allowlist and domain not in self.config.domain_allowlist:
            ...
            if self.domain_request_counts.get(domain, 0) >= self.config.max_pages_per_domain:
            ...
            if not is_safe and warnings:
                ...
                if self.config.quarantine_suspicious:
                    result.error = f"URL failed security scan: {'; '.join(warnings[:2])}"
                    return False
    ...
    async def _fetch_content(self, url: str, result: CrawlResult, **options):
        ...
        allowed_content_types = options.get('allowed_content_types', self.config.allowed_content_types)
