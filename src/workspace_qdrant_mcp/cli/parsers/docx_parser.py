...
class DocxParser(DocumentParser):
    ...
    async def parse(self, file_path: str | Path, **options: Any) -> ParsedDocument:
        ...
        try:
            ...
            text_content = await self._extract_text_content(
                doc, include_tables, include_headers_footers, table_format, preserve_formatting
            )
    ...
    async def _extract_metadata(self, doc: Document) -> dict[str, str | int | float | bool]:
        ...
        has_images = any(
            run._element.xpath('.//a:blip', namespaces={'a': 'http://schemas.openxmlformats.org/drawingml/2006/main'})
        ...
        has_hyperlinks = any(
            para._element.xpath('.//w:hyperlink', namespaces={'w': 'http://schemas.openxmlformats.org/wordprocessingml/2006/main'})
            for para in doc.paragraphs
        )
    ...
    async def _extract_main_content(
        ...
        for element in doc.element.body:
            if element.tag.endswith('p'):  # Paragraph
                ...
                text = await self._extract_paragraph_text(paragraph, preserve_formatting)
                if text.strip():
    ...
    async def _extract_paragraph_text(self, paragraph: Paragraph, preserve_formatting: bool) -> str:
