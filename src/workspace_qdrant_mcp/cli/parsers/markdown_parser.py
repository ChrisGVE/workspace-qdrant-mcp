...
class MarkdownParser(DocumentParser):
        ...
        try:
            ...
            if HAS_MARKDOWN:
                parsed_content, structure_info = await self._parse_with_markdown_lib(
                    content, preserve_structure, include_code_blocks, 
                    include_links, extract_toc
                )
    ...
    async def _parse_with_markdown_lib(
        ...
        if preserve_structure:
            text_content = self._convert_to_structured_text(content, include_code_blocks, include_links)
        else:
            text_content = self._strip_markdown(content, include_code_blocks, include_links)
        ...
        for line in lines:
            ...
            if line.startswith('```'):
                ...
                if include_code_blocks:
                    text_lines.append(f"[CODE BLOCK {'START' if in_code_block else 'END'}]")
                continue
