...
class EpubParser(DocumentParser):
    ...
    async def parse(self, file_path: str | Path, **options: Any) -> ParsedDocument:
        ...
        try:
            ...
            logger.info(f"Successfully parsed EPUB: {file_path.name} "
    ...
    async def _extract_metadata(self, book: epub.EpubBook) -> dict[str, str | int | float | bool]:
        ...
        if book.get_metadata('DC', 'description'):
            ...
            metadata['description'] = description[:500] + '...' if len(description) > 500 else description
        ...
        for item in items:
            try:
                ...
                if len(text) > max_chapter_size:
                    text = text[:max_chapter_size] + '\n\n[Chapter content truncated...]'
                
                if text.strip():
                    chapters.append(text)
                    
            except Exception as e:
                logger.warning(f"Failed to extract content from chapter {item.get_name()}: {e}")
                continue
