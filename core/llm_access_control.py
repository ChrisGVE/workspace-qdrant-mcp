...
try:
    ...
    from .config import McpConfig, DaemonConfig
except ImportError:
...
class LLMAccessController:
    ...
    def __init__(self, config: Optional[Union[McpConfig, DaemonConfig]] = None):
        """
        ...
        """
        self.config = config
        self.classifier = collection_types.CollectionTypeClassifier()
    ...
    def _validate_create_access(self, name: str, info: collection_types.CollectionInfo) -> None:
        ...
        if info.type == collection_types.CollectionType.SYSTEM:
        ...
        if info.type == collection_types.CollectionType.LIBRARY:
        ...
        if info.type == collection_types.CollectionType.GLOBAL:
        ...
        if info.type == collection_types.CollectionType.PROJECT:
        ...
        if info.type == collection_types.CollectionType.UNKNOWN:
            violation = AccessViolation(
                violation_type=AccessViolationType.INVALID_COLLECTION_NAME,
                collection_name=name,
                operation="create",
    ...
    def _validate_delete_access(self, name: str, info: collection_types.CollectionInfo) -> None:
    ...
    def _validate_write_access(self, name: str, info: collection_types.CollectionInfo) -> None:
    ...
    def get_forbidden_collection_patterns(self) -> List[str]:
        ...
        """
        return [
            f"{collection_types.SYSTEM_PREFIX}* (system collections - CLI managed)",
            f"{collection_types.LIBRARY_PREFIX}* (library collections - CLI managed)", 
            f"Global names: {', '.join(collection_types.GLOBAL_COLLECTIONS)}",
    ...
    def suggest_collection_name(self, intended_name: str, purpose: str = "general") -> List[str]:
        ...
        try:
            normalized_base = collection_naming.normalize_collection_name_component(intended_name)
        except (ValueError, TypeError):
            normalized_base = "myproject"
        ...
        for suffix in suffixes:
            try:
                suggestion = collection_naming.build_project_collection_name(normalized_base, suffix)
    ...
    def _build_forbidden_patterns(self) -> Dict[str, str]:
        ...
        # System patterns
        patterns[f"^{re.escape(collection_types.SYSTEM_PREFIX)}"] = "System collections are CLI-managed only"
        
        # Library patterns  
        patterns[f"^{re.escape(collection_types.LIBRARY_PREFIX)}(?!{re.escape(collection_types.SYSTEM_PREFIX)})"] = "Library collections are CLI-managed only"
        
        # Global collection names
        for global_name in collection_types.GLOBAL_COLLECTIONS:
    ...
    def _suggest_alternatives_for_system(self, name: str) -> List[str]:
        ...
        if name.startswith(collection_types.SYSTEM_PREFIX):
            base_name = name[len(collection_types.SYSTEM_PREFIX):]
    ...
    def _suggest_alternatives_for_library(self, name: str) -> List[str]:
        ...
        if name.startswith(collection_types.LIBRARY_PREFIX):
            base_name = name[len(collection_types.LIBRARY_PREFIX):]
            return self.suggest_collection_name(base_name, "general")
        return self.suggest_collection_name(name, "general")
...
def can_llm_create_collection(name: str, config: Optional[Union[McpConfig, DaemonConfig]] = None) -> bool:
    """
...
def can_llm_delete_collection(name: str, config: Optional[Union[McpConfig, DaemonConfig]] = None) -> bool:
    """
...
def validate_llm_collection_access(operation: str, name: str, config: Optional[Union[McpConfig, DaemonConfig]] = None) -> None:
    """
...
def get_forbidden_collection_patterns(config: Optional[Union[McpConfig, DaemonConfig]] = None) -> List[str]:
    """
