...
- Inspected key Python modules (`workspace_qdrant_mcp`, `common`, `wqm_cli`), the Rust daemon sources (`rust-engine`, `src/rust/daemon` workspace), and configuration assets.
...
- Rust side currently has two parallel implementations: `rust-engine` (legacy, built into the `memexd` binary) and a newer modular workspace under `src/rust/daemon` (`workspace-qdrant-core`, `workspace-qdrant-grpc`, Python bindings).
...
- `DaemonClient.ingest_text` expects a functioning `DocumentService` (`src/python/common/core/daemon_client.py:306-365`), but the Rust daemon compiled from `rust-engine` does not expose that service, leading to the fallback discussed above.
...
- Service management assumes a `memexd` binary located under user paths; `wqm service install --build` compiles from `rust-engine` (`src/python/wqm_cli/cli/commands/service.py:49-210`). The binary produced by `rust-engine` is `workspace-qdrant-daemon` (`rust-engine/Cargo.toml`), so the default build path does not match the expected filename, leading to install failures unless renamed.
...
### Rust Daemon – Legacy (`rust-engine`)

- Only the `SystemService` gRPC endpoint is registered; other services remain commented out (`rust-engine/src/grpc/services/mod.rs:3-13`). Consequently, Python clients cannot reach collection or document operations.
- `SystemService` returns canned metrics instead of live health data (`rust-engine/src/grpc/services/system_service.rs:42-118`), limiting observability.
- Document processing builds placeholder embeddings and writes everything into a fixed collection (`rust-engine/src/daemon/processing.rs:143-218`). There is no integration with a real embedding model, no chunking control, and collection naming ignores project metadata.
- File-watcher integration is scaffolded but not wired to ingestion queues (`rust-engine/src/daemon/watcher.rs`), and queue-related modules exist but are unused.
- Runtime initialization wires auto-watch creation from configuration (`rust-engine/src/daemon/mod.rs:134-188`), yet failure paths only log and proceed; there is no backoff strategy or health accounting.
...
- This newer workspace is not referenced by the CLI or the Python MCP components; the build tooling still targets `rust-engine`, so the mature modules remain unused.
...
- CLI build/deploy scripts point at `rust-engine`, while the new Rust workspace has separate tooling (`src/rust/daemon/justfile`, benches). Aligning these build pipelines is necessary before packaging.
...
- **Fragmented Rust code bases** – Maintaining both `rust-engine` and the newer `workspace-qdrant-*` crates dilutes effort and confuses build tooling.
...
1. **Unify the daemon implementation**: Decide between `rust-engine` and the `src/rust/daemon` workspace. Expose the full `workspace_daemon` protocol (DocumentService, CollectionService) and align the build outputs with CLI expectations.
...
6. **Align packaging & naming**: Ensure the Rust build outputs `memexd` (or update CLI expectations), and wire the new Rust workspace into the packaging pipeline if it replaces `rust-engine`.
