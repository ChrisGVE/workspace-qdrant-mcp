...
main() {
	...
	# Download daemon + MCP server (unless --cli-only)
	if [[ "$CLI_ONLY" == false ]]; then
		download_binary "memexd" "$target" "$VERSION" "$TEMP_DIR"
		download_binary "workspace-qdrant-mcp" "$target" "$VERSION" "$TEMP_DIR"
	fi
	...
	if [[ "$CLI_ONLY" == false ]] && [[ -f "$TEMP_DIR/workspace-qdrant-mcp" ]]; then
		mv "$TEMP_DIR/workspace-qdrant-mcp" "$BIN_DIR/"
	...
	if [[ -x "$BIN_DIR/workspace-qdrant-mcp" ]]; then
