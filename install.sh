...
#
# Builds and installs the CLI (wqm), daemon (memexd), and Rust MCP server
# (workspace-qdrant-mcp). All binaries are self-contained with ONNX Runtime
# statically linked.
#
# Usage:
#   ./install.sh [OPTIONS]
...
build_rust() {
	...
	if [ "$CLI_ONLY" = true ]; then
		...
		if cargo build --release -p mcp-server; then
			success "MCP server built successfully"
		else
			error "MCP server build failed. Check error messages above."
		fi
	fi
...
install_binaries() {
	...
	# MCP server (if built)
	if [ "$CLI_ONLY" = false ] && [ -f src/rust/target/release/workspace-qdrant-mcp ]; then
		cp src/rust/target/release/workspace-qdrant-mcp "$BIN_DIR/"
		chmod 755 "$BIN_DIR/workspace-qdrant-mcp"
		success "Installed workspace-qdrant-mcp"
	fi
}
...
verify_installation() {
	...
	# Test MCP server
	if [ -f "$BIN_DIR/workspace-qdrant-mcp" ]; then
		MCP_VERSION=$("$BIN_DIR/workspace-qdrant-mcp" --version 2>/dev/null || echo "unknown")
		success "workspace-qdrant-mcp version: $MCP_VERSION"
	else
		warn "workspace-qdrant-mcp binary not found at $BIN_DIR/workspace-qdrant-mcp"
	fi
}
...
print_summary() {
	...
	if [ -f "$BIN_DIR/workspace-qdrant-mcp" ]; then
		echo "  - workspace-qdrant-mcp (MCP server): $BIN_DIR/workspace-qdrant-mcp"
	fi
	echo ""
	...
	echo "  2. Start daemon: wqm service install && wqm service start"
	echo "  3. Configure MCP client — Claude Desktop (claude_desktop_config.json):"
	echo '     { "mcpServers": { "workspace-qdrant-mcp": { "command": "workspace-qdrant-mcp",'
	echo '       "env": { "QDRANT_URL": "http://localhost:6333" } } } }'
	echo "  4. Or Claude Code: claude mcp add workspace-qdrant-mcp -- workspace-qdrant-mcp"
	echo "  5. Use CLI: wqm --help"
	echo ""
}
