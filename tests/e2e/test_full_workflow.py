...
class TestFullWorkflowE2E:
    ...
    async def test_complete_document_workflow(self, mock_config, mock_qdrant_client):
        ...
        with patch('workspace_qdrant_mcp.core.client.QdrantClient', return_value=mock_qdrant_client):
            ...
            with patch.object(client.project_detector, 'get_project_info') as mock_project_info:
                ...
                client.embedding_service.generate_embeddings = AsyncMock(return_value={
                    "dense": [0.1] * 384,
                    "sparse": {"indices": [1, 5, 10], "values": [0.8, 0.6, 0.9]}
                })
                
                # Mock collection manager
                with patch('workspace_qdrant_mcp.core.client.WorkspaceCollectionManager') as mock_collection_manager_class, \
    ...
    async def test_project_detection_workflow(self, temp_git_repo_with_submodules):
        ...
        with patch('workspace_qdrant_mcp.utils.project_detection.git.Repo') as mock_repo_class:
            mock_repo = MagicMock()
    ...
    async def test_hybrid_search_workflow(self, mock_config, mock_qdrant_client):
        ...
        dense_results = [
            MagicMock(id="doc1", score=0.9, payload={"content": "Python programming guide"}),
            MagicMock(id="doc2", score=0.8, payload={"content": "Machine learning basics"})
        ]
        
        sparse_results = [
            MagicMock(id="doc1", score=0.85, payload={"content": "Python programming guide"}),
            MagicMock(id="doc3", score=0.75, payload={"content": "Web development with FastAPI"})
        ]
    ...
    async def test_scratchbook_workflow(self, mock_workspace_client):
        ...
        # Setup mock client
        mock_workspace_client.get_embedding_service.return_value.generate_embeddings = AsyncMock(
            return_value={
                "dense": [0.1] * 384,
    ...
    @pytest.mark.e2e
    @pytest.mark.slow
    async def test_large_document_processing_workflow(self, mock_config, mock_qdrant_client):
        ...
        with patch('workspace_qdrant_mcp.core.client.QdrantClient', return_value=mock_qdrant_client):
            ...
            mock_qdrant_client.upsert = AsyncMock(return_value=MagicMock(
                status="completed",
                operation_id=123
            ))
    ...
    async def test_multi_project_workspace_workflow(self, mock_config):
        ...
        with patch.object(detector, '_find_git_root', return_value="/path/to/main-project"), \
             patch.object(detector, '_get_git_remote_url', return_value="https://github.com/testuser/main-project.git"), \
             patch('workspace_qdrant_mcp.utils.project_detection.git.Repo') as mock_repo_class:
            ...
            for i in range(5):
                ...
                if i < 3:
                    mock_submodule.url = f"https://github.com/testuser/subproject{i}.git"
                else:
                    mock_submodule.url = f"https://github.com/otheruser/subproject{i}.git"
    ...
    async def test_cleanup_workflow(self, mock_config, mock_qdrant_client):
        ...
        with patch('workspace_qdrant_mcp.core.client.QdrantClient', return_value=mock_qdrant_client):
            ...
            # Initialize client
            with patch.object(client.project_detector, 'get_project_info'), \
