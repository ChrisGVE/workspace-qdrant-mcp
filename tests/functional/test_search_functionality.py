...
class TestSearchFunctionality:
    ...
    async def test_symbol_search_recall(self):
        ...
        for gt_case in symbol_queries:
            ...
            if gt_case['query'] in [s['name'] for s in self.test_data['symbols']]:
                assert metrics.precision >= 0.5, f"Symbol search precision too low: {metrics.precision}"
        ...
        if symbol_metrics:
            ...
            assert avg_precision >= 0.3, "Symbol search average precision should be ≥ 30%"
    ...
    async def test_semantic_search_quality(self):
        ...
        if semantic_metrics:
            ...
            assert avg_recall >= 0.2, "Semantic search recall should be ≥ 20%"
    ...
    async def test_exact_match_search_precision(self):
        ...
        for gt_case in exact_queries:
            ...
            if metrics.total_results > 0:
                assert metrics.precision >= 0.6, f"Exact search precision too low: {metrics.precision}"
        ...
        if exact_metrics:
            ...
            assert avg_precision >= 0.5, "Exact search average precision should be ≥ 50%"
    ...
    def test_export_comprehensive_search_report(self):
        ...
        if aggregate_metrics:
            ...
            if summary.get('total_queries', 0) > 5:
                assert summary.get('avg_precision', 0) >= 0.2, "Overall precision should be ≥ 20%"
