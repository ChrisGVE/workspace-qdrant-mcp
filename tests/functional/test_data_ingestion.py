...
from ..fixtures.test_data_collector import TestDataCollector, CodeSymbol, CodeChunk
...
class TestRealDataIngestion:
    ...
    async def setup_test_environment(self, mock_config, tmp_path):
        ...
        print(f"📊 Collected test data: {len(self.test_data['symbols'])} symbols, "
    ...
    async def test_ingest_python_source_files(self, mock_workspace_client):
        ...
        assert len(code_chunks) > 0, "Should have collected code chunks from real source"
        ...
        with TimedOperation(self.benchmarker, "code_ingestion"):
            for i, chunk in enumerate(code_chunks[:10]):  # Test first 10 chunks
        ...
        file_path_points = [p for p in self.upserted_points if p['payload'].get('file_path')]
    ...
    async def test_ingest_documentation_content(self, mock_workspace_client):
        ...
        doc_points = [p for p in self.upserted_points 
        ...
        markdown_points = [p for p in doc_points 
                          if p['payload'].get('file_type') == '.md']
    ...
    async def test_ingest_function_symbols(self, mock_workspace_client):
        ...
        function_symbols = [
            symbol for symbol in self.test_data['symbols']
        ...
        with TimedOperation(self.benchmarker, "function_symbol_ingestion"):
            for symbol in function_symbols[:8]:  # Test first 8 functions
                ...
                if symbol['docstring']:
                    content_parts.append(f'"""'  f'{symbol["docstring"]}"""')
                if symbol['source_code']:
                    content_parts.append(symbol['source_code'][:500])  # Truncate long code
        ...
        function_points = [p for p in self.upserted_points 
                          if p['payload'].get('symbol_type') in ['function', 'async_function']]
        assert len(function_points) >= 8
        
        # Verify function-specific metadata
        functions_with_docstrings = [p for p in function_points 
                                   if p['payload'].get('has_docstring') is True]
        assert len(functions_with_docstrings) > 0, "Should have functions with docstrings"
        
        # Verify parent class relationships for methods
        methods_in_classes = [p for p in function_points 
                            if p['payload'].get('parent_class')]
    ...
    async def test_ingest_class_symbols(self, mock_workspace_client):
        ...
        class_points = [p for p in self.upserted_points 
    ...
    async def test_chunking_strategy_effectiveness(self, mock_workspace_client):
        ...
        chunking_points = [p for p in self.upserted_points 
        ...
        symbols_per_chunk = [len(p['payload'].get('symbols', [])) for p in chunking_points]
        chunks_with_symbols = [count for count in symbols_per_chunk if count > 0]
        
        symbol_coverage = len(chunks_with_symbols) / len(chunking_points) if chunking_points else 0
    ...
    async def test_ingestion_performance_benchmarks(self, mock_workspace_client):
        ...
        assert single_benchmark.mean_time_ms < 100, "Single document ingestion should be < 100ms"
        ...
        assert batch_speedup > 2.0, f"Batch ingestion should be >2x faster (got {batch_speedup:.1f}x)"
    ...
    async def test_metadata_preservation_and_indexing(self, mock_workspace_client):
        ...
        test_chunks = [
            chunk for chunk in self.test_data['chunks'][:15]  # Mix of different types
        ]
        ...
        metadata_points = [p for p in self.upserted_points 
                          if p['collection'] == collection_name]
        ...
        for point in metadata_points:
            expected_count = len([c for c in test_chunks if c['id'] == point['id'].split('_')[-1]][0]['symbols'])
    ...
    def test_collected_data_quality(self):
        ...
        assert len(self.test_data['symbols']) >= 10, "Should collect at least 10 symbols"
        assert len(self.test_data['chunks']) >= 20, "Should collect at least 20 chunks"
        assert len(self.test_data['ground_truth']) >= 5, "Should generate at least 5 test cases"
        ...
        assert len(found_types) >= 2, f"Should find multiple symbol types, got: {symbol_types}"
        ...
        for gt in self.test_data['ground_truth'][:5]:  # Check first 5
            ...
            assert gt['query_type'] in ['symbol', 'semantic', 'exact'], "Valid query type"
            assert len(gt['expected_results']) > 0, "Should have expected results"
        
        print(f"✅ Test data quality verified:")
    ...
    async def test_full_codebase_ingestion_simulation(self, mock_workspace_client):
        ...
        total_start_time = self.benchmarker._active_timers.get('full_ingestion', 0)
        ...
        assert full_ingestion_time < 30000, "Full ingestion should complete in < 30 seconds"
