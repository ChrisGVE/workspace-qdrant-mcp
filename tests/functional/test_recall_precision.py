...
from ..fixtures.test_data_collector import TestDataCollector, SearchGroundTruth
from ..utils.metrics import RecallPrecisionMeter, SearchMetrics, PerformanceBenchmarker
...
class TestRecallPrecision:
    ...
    def _index_test_data(self):
        ...
        self.symbol_index = {symbol['name']: symbol for symbol in self.test_data['symbols']}
    ...
    async def _create_advanced_search_mock(self):
        ...
        mock_client.list_collections.return_value = ["test_workspace_docs", "test_workspace_code"]
        
        # Advanced search simulation
        async def advanced_search_simulation(query: str, collections=None, mode="hybrid", limit=10, score_threshold=0.0):
            return self._simulate_content_based_search(query, collections, mode, limit, score_threshold)
    ...
    def _simulate_content_based_search(self, query: str, collections=None, mode="hybrid", limit=10, score_threshold=0.0):
        ...
        for chunk in self.test_data['chunks']:
            ...
            if mode == 'hybrid':
                if 'function' in query_lower or 'class' in query_lower or 'method' in query_lower:
                    if chunk_type == 'code':
                        score += 0.3
    ...
    async def test_symbol_search_precision_recall(self):
        ...
        avg_precision_at_1 = statistics.mean(m.precision_at_k.get(1, 0) for m in symbol_metrics)
        avg_precision_at_5 = statistics.mean(m.precision_at_k.get(5, 0) for m in symbol_metrics)
        
        print(f"📊 Symbol search quality results:")
        ...
        print(f"  High precision queries (≥70%): {len(high_precision_queries)}/{len(symbol_metrics)}")
    ...
    async def test_semantic_search_quality_comprehensive(self):
        ...
        semantic_ground_truth = [gt for gt in self.test_data['ground_truth'] if gt['query_type'] == 'semantic']
        ...
        print(f"📊 Semantic search quality results:")
        print(f"  Average precision: {avg_precision:.3f}")
        print(f"  Average recall: {avg_recall:.3f}")
        print(f"  Average F1 score: {avg_f1:.3f}")
        ...
        # Calculate recall@k
        avg_recall_at_5 = statistics.mean(m.recall_at_k.get(5, 0) for m in semantic_metrics)
        avg_recall_at_10 = statistics.mean(m.recall_at_k.get(10, 0) for m in semantic_metrics)
    ...
    async def test_hybrid_search_quality_vs_individual_modes(self):
        ...
        for query, expected_type in test_queries:
            ...
            for chunk in self.test_data['chunks']:
                ...
                if (query_lower in content_lower or 
                    any(query_lower in symbol or symbol in query_lower for symbol in chunk_symbols)):
            ...
            print(f"  '{query}': Semantic F1={semantic_metrics.f1_score:.3f}, Hybrid F1={hybrid_metrics.f1_score:.3f}")
        ...
        semantic_avg_recall = statistics.mean(m.recall for m in mode_results['semantic'])
        hybrid_avg_recall = statistics.mean(m.recall for m in mode_results['hybrid'])
        
        semantic_avg_precision = statistics.mean(m.precision for m in mode_results['semantic'])
        hybrid_avg_precision = statistics.mean(m.precision for m in mode_results['hybrid'])
        
        print(f"📊 Mode comparison results:")
        print(f"  Semantic - F1: {semantic_avg_f1:.3f}, P: {semantic_avg_precision:.3f}, R: {semantic_avg_recall:.3f}")
        print(f"  Hybrid   - F1: {hybrid_avg_f1:.3f}, P: {hybrid_avg_precision:.3f}, R: {hybrid_avg_recall:.3f}")
        
        # Analyze which mode performs better for different query types
        symbol_queries = [(q, s, h) for (q, qt), s, h in zip(test_queries, mode_results['semantic'], mode_results['hybrid']) if qt == 'symbol']
        semantic_queries = [(q, s, h) for (q, qt), s, h in zip(test_queries, mode_results['semantic'], mode_results['hybrid']) if qt == 'semantic']
        
        if symbol_queries:
            symbol_semantic_f1 = statistics.mean(s.f1_score for _, s, _ in symbol_queries)
            symbol_hybrid_f1 = statistics.mean(h.f1_score for _, _, h in symbol_queries)
            print(f"  Symbol queries - Semantic: {symbol_semantic_f1:.3f}, Hybrid: {symbol_hybrid_f1:.3f}")
        
        if semantic_queries:
            sem_semantic_f1 = statistics.mean(s.f1_score for _, s, _ in semantic_queries)
            sem_hybrid_f1 = statistics.mean(h.f1_score for _, _, h in semantic_queries)
            print(f"  Semantic queries - Semantic: {sem_semantic_f1:.3f}, Hybrid: {sem_hybrid_f1:.3f}")
        
        # Hybrid should perform competitively overall
        improvement_ratio = hybrid_avg_f1 / semantic_avg_f1 if semantic_avg_f1 > 0 else 1
        print(f"  Hybrid improvement: {improvement_ratio:.2f}x")
        
        # Don't require hybrid to always be better, but should be competitive
        assert improvement_ratio >= 0.8, f"Hybrid should be competitive with semantic (≥0.8x, got {improvement_ratio:.2f}x)"
    ...
    async def test_ranking_quality_and_relevance_distribution(self):
        ...
        for query in ranking_test_queries:
            ...
            for result in top_3_results:
                ...
                is_relevant = (
                    any(word in content_lower for word in query_words) or
                    any(any(word in symbol.lower() for word in query_words) for symbol in symbols) or
            ...
            print(f"  '{query}':")
            print(f"    Results: {len(results)}, Ordered: {'✓' if is_properly_ordered else '✗'}")
            print(f"    Score range: {score_range:.3f}, Top-3 relevance: {top_3_relevance_ratio:.2f}")
        
        # Overall ranking quality metrics
        properly_ordered_ratio = sum(1 for a in ranking_analyses if a['properly_ordered']) / len(ranking_analyses)
        avg_top_3_relevance = statistics.mean(a['top_3_relevance_ratio'] for a in ranking_analyses)
        avg_score_range = statistics.mean(a['score_range'] for a in ranking_analyses)
        
        print(f"📊 Overall ranking quality:")
        ...
        assert properly_ordered_ratio >= 0.8, f"≥80% of results should be properly ordered (got {properly_ordered_ratio:.2f})"
        assert avg_top_3_relevance >= 0.5, f"Top-3 results should be ≥50% relevant (got {avg_top_3_relevance:.3f})"
        assert avg_score_range >= 0.1, f"Score range should be ≥0.1 for discrimination (got {avg_score_range:.3f})"
    ...
    async def test_query_type_specific_optimization(self):
            ...
            for query in queries:
                ...
                for chunk in self.test_data['chunks']:
                    ...
                    if query_type == 'exact_match':
                        ...
                        if (query_lower in content_lower or 
                            any(query_lower == symbol for symbol in symbols)):
                            expected_results.add(chunk['id'])
                    elif query_type == 'partial_match':
                        # Partial matches should find related content
                        query_words = query_lower.split()
                        if (any(word in content_lower for word in query_words) or
                            any(any(word in symbol for word in query_words) for symbol in symbols)):
                            expected_results.add(chunk['id'])
                    else:  # conceptual
        ...
        print(f"📊 Query type optimization analysis:")
        ...
        for query_type, performance in query_type_performance.items():
            assert performance['f1'] >= 0.15, f"{query_type} F1 should be ≥15% (got {performance['f1']:.3f})"
    ...
    async def test_cross_validation_quality_consistency(self):
        ...
        for fold_idx, fold_data in enumerate(folds):
            ...
            print(f"    Fold {fold_idx + 1}: P={fold_avg_precision:.3f}, R={fold_avg_recall:.3f}, F1={fold_avg_f1:.3f}")
        ...
        if len(fold_results) >= 2:
            ...
            assert precision_std < 0.2, f"Precision should be consistent across folds (std < 0.2, got {precision_std:.3f})"
            assert f1_std < 0.2, f"F1 score should be consistent across folds (std < 0.2, got {f1_std:.3f})"
    ...
    def _export_final_quality_report(self):
        ...
        print(f"📊 Quality reports exported:")
        ...
        if summary:
            ...
            print(f"  Average search time: {summary.get('avg_search_time_ms', 0):.1f}ms")
