...
from workspace_qdrant_mcp.server import (
    app, workspace_status, list_workspace_collections, search_workspace_tool,
    ...
    delete_scratchbook_note_tool, hybrid_search_advanced_tool
)
...
class TestMCPIntegration:
    ...
    async def setup_mcp_environment(self, mock_config, tmp_path):
        ...
        print(f"🔧 MCP integration test setup completed")
    ...
    async def _create_comprehensive_mock_client(self):
        ...
        mock_client.get_status.return_value = {
            ...
            "collections_count": 3,
            "workspace_collections": ["test_workspace_docs", "test_workspace_code", "test_workspace_scratchbook"],
        ...
        mock_client.list_collections.return_value = [
            "test_workspace_docs", "test_workspace_code", "test_workspace_scratchbook"
        ]
        ...
        async def mock_add_document(content, collection, metadata=None, document_id=None, chunk_text=True):
        ...
        async def mock_get_document(document_id, collection, include_vectors=False):
            if document_id in self.mock_documents:
                ...
                if include_vectors:
                    doc["vectors"] = {"dense": [0.1] * 384, "sparse": {"indices": [1, 5], "values": [0.8, 0.6]}}
                return {"success": True, "document": doc}
            return {"success": False, "error": "Document not found"}
        
        # Mock search operations with realistic results based on test data
        async def mock_search_workspace(query, collections=None, mode="hybrid", limit=10, score_threshold=0.7):
            ...
            for chunk in self.test_data['chunks'][:limit]:
                if any(word in chunk['content'].lower() for word in query_lower.split()):
                    results.append({
                        ...
                        "collection": collections[0] if collections else "test_workspace_docs"
                    })
        ...
        async def mock_scratchbook_manager_search(query, note_types=None, tags=None, project_name=None, limit=10, mode="hybrid"):
            matching_notes = []
        ...
        async def mock_scratchbook_manager_update(content, note_id=None, title=None, tags=None, note_type="note"):
        ...
        async def mock_scratchbook_manager_list(project_name=None, note_type=None, tags=None, limit=50):
            filtered_notes = []
            for note_id, note in self.mock_scratchbook_notes.items():
                include = True
    ...
    async def test_workspace_status_tool(self):
        ...
        print(f"✅ Workspace status: {result['collections_count']} collections, project: {result['current_project']}")
    ...
    async def test_list_workspace_collections_tool(self):
        ...
        expected_collections = ["test_workspace_docs", "test_workspace_code", "test_workspace_scratchbook"]
    ...
    async def test_add_document_tool_comprehensive(self):
        ...
        for i, doc_data in enumerate(test_documents):
            ...
            print(f"✅ Added document {result['document_id']} to {result['collection']}")
    ...
    async def test_search_workspace_tool_comprehensive(self):
        ...
        test_docs = [
            ("Python client initialization", "test_workspace_code", {"language": "python", "type": "client"}),
            ("FastMCP server configuration", "test_workspace_docs", {"framework": "fastmcp", "type": "config"}),
            ("Qdrant collection management", "test_workspace_code", {"database": "qdrant", "type": "management"}),
            ("Embedding vector generation", "test_workspace_docs", {"ml": True, "type": "embedding"})
        ]
        ...
        for search_params in search_tests:
            async with AsyncTimedOperation(self.benchmarker, f"search_{search_params['mode']}"):
            ...
            print(f"✅ Search '{search_params['query']}' ({search_params['mode']}): {len(results)} results")
    ...
    async def test_search_by_metadata_tool(self):
        ...
        metadata_test_docs = [
            ("Code documentation", {"doc_type": "code", "language": "python", "category": "api"}),
            ("User guide", {"doc_type": "guide", "audience": "users", "category": "tutorial"}),
            ("Development notes", {"doc_type": "code", "language": "python", "category": "internal"}),
            ("API reference", {"doc_type": "reference", "audience": "developers", "category": "api"})
        ]
    ...
    async def test_scratchbook_tools_comprehensive(self):
        ...
        research_notes = await list_scratchbook_notes_tool(note_type="research", limit=5)
        ...
        for search_params in search_tests:
            ...
            results = search_result.get("results", [])
            print(f"✅ Scratchbook search '{search_params['query']}': {len(results)} results")
    ...
    async def test_hybrid_search_advanced_tool(self):
        ...
        for search_params in hybrid_search_tests:
            async with AsyncTimedOperation(self.benchmarker, f"hybrid_search_{search_params['fusion_method']}"):
            ...
            if result.get("error"):
                ...
                print(f"⚠️  Advanced hybrid search returned expected error: {result['error']}")
            else:
                # If successful, verify structure
                assert "results" in result or "matches" in result
                print(f"✅ Advanced hybrid search completed: {search_params['fusion_method']}")
    ...
    async def test_mcp_tool_performance_benchmarks(self):
        ...
        operations_to_benchmark = [
            ...
            ("search_workspace", search_workspace_tool, {
        ...
        for op_name, op_func, op_kwargs in operations_to_benchmark:
            benchmark = await self.benchmarker.benchmark_async_operation(
                f"mcp_{op_name}",
                lambda: op_func(**op_kwargs),
                iterations=5
            )
            
            print(f"  {op_name}: {benchmark.mean_time_ms:.1f}ms avg, {benchmark.operations_per_second:.1f} ops/sec")
            
            # Performance assertions
            assert benchmark.mean_time_ms < 1000, f"{op_name} should be < 1000ms"
            assert benchmark.operations_per_second > 1, f"{op_name} should handle > 1 op/sec"
        ...
        assert concurrent_benchmark.mean_time_ms < 2000, "Concurrent operations should be < 2s"
    ...
    async def test_error_handling_and_edge_cases(self):
        ...
        empty_search_result = await search_workspace_tool(
            query="",
            limit=5
        )
        ...
        large_limit_result = await search_workspace_tool(
            query="test",
            limit=1000
        )
    ...
    async def test_data_consistency_across_operations(self):
        ...
        add_result = await add_document_tool(
            ...
            collection="test_workspace_docs",
            metadata={"test_type": "consistency", "unique_id": "CONSISTENCY_TEST_12345"},
        ...
        assert retrieved_doc.get("metadata", {}).get("unique_id") == "CONSISTENCY_TEST_12345"
        
        # Search for document by content
        search_result = await search_workspace_tool(
            query="CONSISTENCY_TEST_12345",
            collections=["test_workspace_docs"],
            limit=5
        )
        ...
        consistency_docs = [r for r in metadata_results if r.get("metadata", {}).get("unique_id") == "CONSISTENCY_TEST_12345"]
        
        if consistency_docs:
            assert len(consistency_docs) > 0, "Document should be found by metadata search"
            print("✅ Document found in metadata search")
        else:
            print("⚠️  Document not found in metadata search (expected in mock environment)")
