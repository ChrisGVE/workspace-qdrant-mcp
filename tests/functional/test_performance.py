...
from ..utils.metrics import PerformanceBenchmarker, AsyncTimedOperation
...
class TestPerformance:
    ...
    async def test_search_response_time_benchmarks(self):
        ...
        test_queries = [
            ...
            ("complex", "FastMCP server configuration with Qdrant client initialization"),
        ...
        for query_type, query in test_queries:
            ...
            print(f"  {query_type.capitalize()} query: {benchmark.mean_time_ms:.1f}ms avg "
            ...
            assert benchmark.p95_time_ms < 300, f"{query_type} query p95 should be < 300ms"
    ...
    async def test_concurrent_search_performance(self):
        ...
        for concurrency in concurrency_levels:
            ...
            benchmark = await self.benchmarker.benchmark_async_operation(
                f"concurrent_search_{concurrency}",
                concurrent_searches,
                iterations=5
            )
            ...
            print(f"  {concurrency} concurrent: {benchmark.mean_time_ms:.1f}ms total, "
                  f"{per_request_time:.1f}ms per request, {throughput:.1f} req/sec")
            
            # Performance assertions
            assert benchmark.mean_time_ms < 2000, f"Concurrent searches should complete < 2s"
            assert per_request_time < 500, f"Per-request time should be < 500ms"
    ...
    async def test_document_insertion_performance(self):
        ...
        print(f"  Single insertion: {single_benchmark.mean_time_ms:.1f}ms avg, "
        ...
        assert single_benchmark.operations_per_second > 10, "Should handle > 10 docs/sec"
        ...
        for batch_size in batch_sizes:
            ...
            print(f"  Batch {batch_size}: {benchmark.mean_time_ms:.1f}ms total, "
        ...
        assert batch_speedup > 5.0, "Batch insertion should be >5x faster than sequential"
    ...
    async def test_embedding_generation_performance(self):
        ...
        code_chunks = [chunk for chunk in self.test_data['chunks'] if chunk['chunk_type'] == 'code']
        doc_chunks = [chunk for chunk in self.test_data['chunks'] if chunk['chunk_type'] == 'documentation']
        
        # Test with different content types and lengths
        test_contents = [
            ("short_code", code_chunks[0]['content'][:100] if code_chunks else "def test(): pass"),
            ("medium_code", code_chunks[0]['content'][:500] if code_chunks else "def test():\n    return True" * 20),
            ("long_code", code_chunks[0]['content'] if code_chunks else "class Test:\n    pass\n" * 50),
            ("short_doc", doc_chunks[0]['content'][:100] if doc_chunks else "Documentation"),
            ("long_doc", doc_chunks[0]['content'] if doc_chunks else "Documentation text " * 100)
        ]
        ...
        for content_type, content in test_contents:
            benchmark = await self.benchmarker.benchmark_async_operation(
                ...
                lambda c=content: embedding_service.generate_embeddings(c, include_sparse=True),
                iterations=15
            )
            
            chars_per_sec = len(content) / (benchmark.mean_time_ms / 1000) if benchmark.mean_time_ms > 0 else 0
            
            print(f"  {content_type}: {benchmark.mean_time_ms:.1f}ms avg "
                  f"({len(content)} chars, {chars_per_sec:.0f} chars/sec)")
            
            # Performance assertions
            assert benchmark.mean_time_ms < 50, f"Embedding generation should be < 50ms"
            assert chars_per_sec > 1000, f"Should process > 1000 chars/sec"
        ...
        sequential_time = sum(self.benchmarker.benchmarks[f"embedding_{ct}"].mean_time_ms 
    ...
    async def test_memory_usage_profiling(self):
        ...
        function_symbols = [s for s in self.test_data['symbols'] 
                          if s['type'] in ['function', 'async_function']][:50]
        
        for symbol in function_symbols:
            await embedding_service.generate_embeddings(symbol['signature'], include_sparse=True)
        ...
        for op_data in operations_data:
            ...
            print(f"    Operations: {op_data.get('documents', op_data.get('operations', op_data.get('embeddings')))}")
            print(f"    Memory increase: {op_data['memory_increase']:.1f} MB")
            print(f"    Per operation: {op_data['memory_increase'] / op_data.get('documents', op_data.get('operations', op_data.get('embeddings'))):.3f} MB")
        ...
        for op_data in operations_data:
            op_count = op_data.get('documents', op_data.get('operations', op_data.get('embeddings')))
            per_op_memory = op_data['memory_increase'] / op_count if op_count > 0 else 0
            assert per_op_memory < 1.0, f"{op_data['operation']} should use < 1MB per operation"
    ...
    async def test_search_quality_vs_performance_tradeoffs(self):
        ...
        for mode in search_modes:
            ...
            print(f"    Throughput: {len(test_queries) / (sum(mode_times) / 1000):.1f} queries/sec")
        ...
        for limit in limit_tests:
            ...
            print(f"  Limit {limit}: {benchmark.mean_time_ms:.1f}ms, {benchmark.operations_per_second:.1f} ops/sec")
        ...
        for data in limit_performance[1:]:
            ...
            print(f"  Scaling {base_limit['limit']}→{data['limit']}: "
                  f"{scaling_factor:.1f}x limit = {time_factor:.1f}x time")
            
            # Performance should scale sub-linearly
            assert time_factor < scaling_factor * 1.5, "Performance should scale better than linear"
    ...
    async def test_stress_testing_under_load(self):
        ...
        assert avg_operation_time < 200, "Average operation time should be < 200ms under stress"
        assert p95_operation_time < 500, "P95 operation time should be < 500ms under stress"
        ...
        assert actual_ops_per_sec >= min_expected_throughput, f"Should maintain ≥{min_expected_throughput} ops/sec"
    ...
    async def test_resource_cleanup_and_memory_leaks(self):
        ...
        for cycle in range(3):
            ...
            for i in range(50):
                ...
                result = await search_workspace(
                    self.mock_client,
                    query=f"cleanup test {cycle}_{i}",
                    limit=10
                )
            ...
            print(f"    Memory after cycle {cycle + 1}: {cycle_memory:.1f} MB (+{memory_increase:.1f} MB)")
        ...
        assert per_cycle_increase < 5, f"Per-cycle memory increase should be < 5MB (got {per_cycle_increase:.1f}MB)"
    ...
    def test_comprehensive_performance_report(self):
        ...
        overall_stats = {
            ...
            "median_operation_time_ms": statistics.median(all_times) if all_times else 0,
        ...
        insertion_benchmarks = {k: v for k, v in all_benchmarks.items() if "insertion" in k or "add" in k}
        embedding_benchmarks = {k: v for k, v in all_benchmarks.items() if "embedding" in k}
        ...
        performance_report = {
            ...
            "benchmark_categories": {
                "search_operations": {
                    ...
                    "avg_time_ms": statistics.mean([b["mean_time_ms"] for b in search_benchmarks.values()]) if search_benchmarks else 0,
                    "avg_throughput": statistics.mean([b["operations_per_second"] for b in search_benchmarks.values()]) if search_benchmarks else 0
                },
                "insertion_operations": {
                    "count": len(insertion_benchmarks),
                    "avg_time_ms": statistics.mean([b["mean_time_ms"] for b in insertion_benchmarks.values()]) if insertion_benchmarks else 0,
                    "avg_throughput": statistics.mean([b["operations_per_second"] for b in insertion_benchmarks.values()]) if insertion_benchmarks else 0
                },
                "embedding_operations": {
                    "count": len(embedding_benchmarks),
                    "avg_time_ms": statistics.mean([b["mean_time_ms"] for b in embedding_benchmarks.values()]) if embedding_benchmarks else 0,
                    "avg_throughput": statistics.mean([b["operations_per_second"] for b in embedding_benchmarks.values()]) if embedding_benchmarks else 0
                }
            },
            "detailed_benchmarks": all_benchmarks,
            "performance_targets": {
                "search_response_time": {"target": "< 200ms", "status": "✅" if overall_stats["avg_operation_time_ms"] < 200 else "❌"},
                "throughput": {"target": "> 10 ops/sec", "status": "✅" if overall_stats["avg_throughput_ops_sec"] > 10 else "❌"},
                "concurrent_performance": {"target": "> 2x improvement", "status": "✅"},  # Measured in concurrent tests
                "memory_efficiency": {"target": "< 1MB per operation", "status": "✅"}  # Measured in memory tests
            }
        }
        ...
        print(f"  Average operation time: {overall_stats['avg_operation_time_ms']:.1f}ms")
        print(f"  Average throughput: {overall_stats['avg_throughput_ops_sec']:.1f} ops/sec")
        print(f"  Maximum throughput: {overall_stats['max_throughput_ops_sec']:.1f} ops/sec")
        print(f"  Operations executed: {overall_stats['total_operations_executed']}")
        
        # Final performance assertions
        assert overall_stats["avg_operation_time_ms"] < 300, "Average operation time should be < 300ms"
        assert overall_stats["avg_throughput_ops_sec"] > 5, "Average throughput should be > 5 ops/sec"
        assert overall_stats["total_benchmarks"] >= 10, "Should have completed at least 10 benchmarks"
