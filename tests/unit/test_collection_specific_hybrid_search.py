...
class TestCollectionSpecificEdgeCases:
    ...
    async def test_empty_collection_results(
        ...
        # Test each collection type (ADR-001: canonical collections have no underscore prefix)
        collection_names = ["myapp-notes", "projects", "libraries", "memory"]
...
class TestMultiCollectionTypeScenarios:
    ...
    async def test_search_across_different_collection_types(
        ...
        """
        # ADR-001: Canonical collections have no underscore prefix
        collection_configs = {
            "myapp-notes": ("user", user_collection_results),
            "projects": ("project", project_collection_results),
            "libraries": ("library", library_collection_results),
