...
class TestWorkspaceCollectionManager:
    ...
    async def test_initialize_workspace_collections_basic(self, collection_manager):
        ...
        collection_manager.client.get_collections.return_value = models.CollectionsResponse(
            collections=[]
        )
        ...
        created_names = [call[1]["collection_name"] for call in collection_manager.client.create_collection.call_args_list]
        assert any("test-project" in name for name in created_names)
    
    @pytest.mark.asyncio
    async def test_initialize_workspace_collections_with_subprojects(self, collection_manager):
    ...
    @pytest.mark.asyncio
    async def test_initialize_workspace_collections_existing_collections(self, collection_manager):
    ...
    @pytest.mark.asyncio
    async def test_initialize_workspace_collections_global_collections(self, collection_manager):
    ...
    async def test_create_collection_failure(self, collection_manager):
        ...
        collection_manager.client.create_collection.side_effect = ResponseHandlingException(
    ...
    async def test_delete_collection_failure(self, collection_manager):
        ...
        collection_manager.client.delete_collection.side_effect = ResponseHandlingException(
    ...
    async def test_collection_exists_exception(self, collection_manager):
        ...
        collection_manager.client.get_collections.side_effect = ResponseHandlingException(
            "Connection error"
        )
    ...
    async def test_get_collection_info_success(self, collection_manager):
        ...
        result = await collection_manager.get_collection_info("test-collection")
        
        assert result is not None
        assert result["name"] == "test-collection"
        assert result["status"] == "GREEN"
        assert result["vectors_count"] == 150
        assert result["points_count"] == 75
    ...
    async def test_get_collection_info_not_found(self, collection_manager):
        ...
        collection_manager.client.get_collection.side_effect = ResponseHandlingException(
            "Collection not found"
        )
        
        result = await collection_manager.get_collection_info("nonexistent-collection")
    ...
    def test_validate_collection_limits_unlimited(self, collection_manager):
        ...
        existing_collections = [f"collection{i}" for i in range(100)]  # Many collections
