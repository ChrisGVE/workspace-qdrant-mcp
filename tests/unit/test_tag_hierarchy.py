...
class TestTagIndexing:
    ...
    async def test_index_tag_simple(self, state_manager):
        ...
        tag = await state_manager.get_tag("myproject.main", "projects")
        assert tag is not None
        assert tag["tag_value"] == "myproject.main"
        assert tag["collection"] == "projects"
    ...
    async def test_index_tag_root_level(self, state_manager):
        ...
        tag = await state_manager.get_tag("myproject", "projects")
        assert tag is not None
    ...
    async def test_index_tag_library_hierarchy(self, state_manager):
        ...
        await state_manager.index_tag("numpy", "libraries", "library")
        await state_manager.index_tag("numpy.linalg", "libraries", "library")
        await state_manager.index_tag("numpy.linalg.norm", "libraries", "library")

        # Verify all tags (plus auto-created parent tags)
        tags = await state_manager.list_tags(collection="libraries")
        ...
        # Verify hierarchy
        deep_tag = await state_manager.get_tag("numpy.linalg.norm", "libraries")
    ...
    async def test_increment_document_count(self, state_manager):
        ...
        # Increment count
        await state_manager.increment_tag_document_count("myproject.main", "projects")
        await state_manager.increment_tag_document_count("myproject.main", "projects")
...
class TestTagFiltering:
    ...
    async def test_list_tags_by_collection(self, state_manager):
        ...
        await state_manager.index_tag("proj1.main", "projects", "project")
        await state_manager.index_tag("lib1.folder", "libraries", "library")

        # Filter by collection (note: parent tags are auto-created)
        project_tags = await state_manager.list_tags(collection="projects")
        ...
        library_tags = await state_manager.list_tags(collection="libraries")
    ...
    async def test_list_tags_by_type(self, state_manager):
        ...
        await state_manager.index_tag("proj1.main", "projects", "project")
        await state_manager.index_tag("lib1.folder", "libraries", "library")
        await state_manager.index_tag("mem1", "memory", "memory")
    ...
    async def test_list_tags_by_parent(self, state_manager):
        ...
        await state_manager.index_tag("numpy", "libraries", "library")
        await state_manager.index_tag("numpy.linalg", "libraries", "library")
        await state_manager.index_tag("numpy.random", "libraries", "library")
        await state_manager.index_tag("pandas", "libraries", "library")
...
class TestWatchedTags:
    ...
    async def test_set_tag_watched(self, state_manager):
        ...
        await state_manager.index_tag("myproject.main", "projects", "project")

        # Mark as watched
        await state_manager.set_tag_watched("myproject.main", "projects", True)

        tag = await state_manager.get_tag("myproject.main", "projects")
        assert tag["watched"] is True

        # Unmark
        await state_manager.set_tag_watched("myproject.main", "projects", False)
        tag = await state_manager.get_tag("myproject.main", "projects")
        assert tag["watched"] is False
    ...
    async def test_get_watched_tags(self, state_manager):
        ...
        await state_manager.index_tag("proj1.main", "projects", "project")
        await state_manager.index_tag("proj1.dev", "projects", "project")
        await state_manager.index_tag("proj2.main", "projects", "project")

        # Mark some as watched
        await state_manager.set_tag_watched("proj1.main", "projects", True)
        await state_manager.set_tag_watched("proj2.main", "projects", True)
...
class TestTagDeletion:
    ...
    async def test_delete_tag(self, state_manager):
        ...
        # Delete
        await state_manager.delete_tag("myproject.main", "projects")
    ...
    async def test_delete_nonexistent_tag(self, state_manager):
        ...
        # Should not raise
        await state_manager.delete_tag("nonexistent.tag", "projects")
...
class TestTagAutoParentCreation:
    ...
    async def test_deep_tag_creates_parents(self, state_manager):
        ...
        # Index a deep tag
        await state_manager.index_tag("a.b.c.d", "libraries", "library")
        ...
        # Verify depths
        a = await state_manager.get_tag("a", "libraries")
        assert a["depth"] == 1
        assert a["parent_tag"] is None

        ab = await state_manager.get_tag("a.b", "libraries")
        assert ab["depth"] == 2
        assert ab["parent_tag"] == "a"

        abcd = await state_manager.get_tag("a.b.c.d", "libraries")
        assert abcd["depth"] == 4
