...
class TestCanonicalCollectionsUsage:
    """Tests verifying canonical collections are properly defined (ADR-001)."""
    ...
    def test_all_collections_no_underscore_prefix(self):
        """Test that canonical collections have no underscore prefix (ADR-001)."""
        for name, collection in CANONICAL_COLLECTIONS.items():
            assert not collection.startswith("_"), f"Collection {name} should NOT start with underscore per ADR-001"
