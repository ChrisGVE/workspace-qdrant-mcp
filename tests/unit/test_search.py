...
class TestSearchWorkspace:
    ...
    @pytest.mark.asyncio
    async def test_search_workspace_no_collections_available(self, mock_workspace_client):
    ...
    async def test_search_workspace_hybrid_success(self, mock_workspace_client, mock_embedding_service):
        ...
        mock_workspace_client.initialized = True
        mock_workspace_client.list_collections.return_value = ["docs", "references"]
        mock_workspace_client.get_embedding_service.return_value = mock_embedding_service
    ...
    @pytest.mark.asyncio
    async def test_search_workspace_specific_collections(self, mock_workspace_client, mock_embedding_service):
        """Test search with specific collections specified."""
        mock_workspace_client.initialized = True
        mock_workspace_client.list_collections.return_value = ["docs", "references", "scratchbook"]
        mock_workspace_client.get_embedding_service.return_value = mock_embedding_service
    ...
    @pytest.mark.asyncio
    async def test_search_workspace_dense_mode(self, mock_workspace_client, mock_embedding_service):
    ...
    @pytest.mark.asyncio
    async def test_search_workspace_sparse_mode(self, mock_workspace_client, mock_embedding_service):
    ...
    @pytest.mark.asyncio
    async def test_search_workspace_no_sparse_embeddings_sparse_mode(self, mock_workspace_client, mock_embedding_service):
    ...
    @pytest.mark.asyncio
    async def test_search_workspace_results_filtering_by_threshold(self, mock_workspace_client, mock_embedding_service):
        ...
        mock_workspace_client.client = mock_client
        mock_client.search.return_value = [
            models.ScoredPoint(id="doc1", score=0.9, version=0, payload={"content": "High score"}),
            models.ScoredPoint(id="doc2", score=0.6, version=0, payload={"content": "Low score"}),
            models.ScoredPoint(id="doc3", score=0.8, version=0, payload={"content": "Medium score"})
        ]
    ...
    @pytest.mark.asyncio
    async def test_search_workspace_results_sorted_by_score(self, mock_workspace_client, mock_embedding_service):
        ...
        mock_workspace_client.client = mock_client
        mock_client.search.return_value = [
            models.ScoredPoint(id="doc2", score=0.8, version=0, payload={"content": "Medium"}),
            models.ScoredPoint(id="doc1", score=0.9, version=0, payload={"content": "High"}),
            models.ScoredPoint(id="doc3", score=0.7, version=0, payload={"content": "Low"})
        ]
    ...
    @pytest.mark.asyncio
    async def test_search_workspace_limit_enforcement(self, mock_workspace_client, mock_embedding_service):
        ...
        mock_results = [
            models.ScoredPoint(id=f"doc{i}", score=0.9 - (i * 0.1), version=0, payload={"content": f"Doc {i}"})
            for i in range(10)
        ]
    ...
    async def test_search_workspace_exception_handling(self, mock_workspace_client):
        ...
        mock_workspace_client.initialized = True
        mock_workspace_client.list_collections.side_effect = Exception("Connection error")
...
class TestSearchCollectionByMetadata:
    ...
    @pytest.mark.asyncio
    async def test_search_by_metadata_client_not_initialized(self, mock_workspace_client):
    ...
    async def test_search_by_metadata_limit_enforcement(self, mock_workspace_client):
        ...
        # Create more results than the limit
        mock_points = [
            MagicMock(id=f"doc{i}", payload={"content": f"Doc {i}", "category": "python"})
            for i in range(10)
        ]
