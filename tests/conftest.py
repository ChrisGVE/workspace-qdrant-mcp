...
def temp_git_repo_with_submodules():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        main_repo.config_writer().set_value("user", "email", "test@example.com").release()
        
        # Add remote origin
        main_repo.create_remote("origin", "https://github.com/testuser/main-project.git")
...
"""
        ...
        (Path(temp_dir) / "libs" / "subproject2" / "file.txt").write_text("subproject2 content")
...
def mock_project_detector():
    ...
    detector.get_project_and_subprojects.return_value = ("test-project", ["subproject1"])
...
def sample_documents():
    ...
    return [
        ...
        },
        {
            ...
            "metadata": {"source": "tutorials", "category": "web", "language": "python"}
        }
    ]
