 ...
 */
export function addBook(shelf: Shelf, book: Book): void {
  if (shelf.books.length >= shelf.capacity) {
    throw new Error(
      `Shelf "${shelf.name}" is at capacity (${shelf.capacity} books)`
    );
  }
 ...
 */
export function removeBook(shelf: Shelf, isbn: string): void {
  const index = shelf.books.findIndex((b) => b.isbn === isbn);
  if (index === -1) {
    throw new Error(
      `Book with ISBN ${isbn} not found on shelf "${shelf.name}"`
    );
  }
 ...
 */
export function findByAuthor(shelf: Shelf, author: string): Book[] {
 ...
 */
export function findByYearRange(
  shelf: Shelf,
  startYear: number,
  endYear: number
): Book[] {
 ...
 */
export function generateReport(shelf: Shelf): string {
  const lines: string[] = [];
  ...
  for (const book of shelf.books) {
    ...
    lines.push(
      `  ${marker} ${book.title} by ${book.author} (${book.year}) - ${book.isbn}`
    );
  }
  lines.push("");
