...
class TestDataCollector:
    ...
    def collect_all_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ground_truth": [self._ground_truth_to_dict(gt) for gt in self.ground_truth],
            "metadata": {
    ...
    def _process_documentation_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if (line.startswith('#') and len(current_chunk) > 1) or len(current_chunk) >= 10:
                    if len(current_chunk) > 1:
                        chunk_content = '\n'.join(current_chunk[:-1] if line.startswith('#') else current_chunk)
    ...
    def _create_code_chunks(self, file_path: Path, content: str, symbols: List[CodeSymbol]) -> None:
    ...
    def _generate_ground_truth(self) -> None:
        ...
        for symbol in self.symbols[:20]:  # Limit for testing
            self.ground_truth.append(SearchGroundTruth(
                query=symbol.name,
                query_type="symbol",
                expected_results=[self._generate_chunk_id(c) for c in self.chunks 
