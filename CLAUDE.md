...
All 8 oversized TypeScript source files have been split (arch-refactor Task 18, done). Additionally, during arch-refactor-2, tool-definitions.ts, tool-builders.ts, list-files.ts, tracked-files-queries.ts, and component-detector.ts were also split. Only 2 marginally oversized files remain: grpc-types-messages.ts (344 lines) and project-detector.ts (321 lines).
...
| `mcp-server/tests/tools/rules.test.ts` | 441 |
