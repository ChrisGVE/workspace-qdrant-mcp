...
**Case-3 embed via reuse + re-key (Option C — IMPLEMENTED 2026-06-21, supersedes the duplication note).** The earlier framing here called for the tagger to replicate `embed_chunks`'s semaphore+batch+sparse assembly ("accepted drift debt, Option B"). That was rejected at implementation: replicating only the sketch above would have DROPPED LSP enrichment, lexicon-BM25 sparse vectors, oversized-chunk splitting, the rich payload (content/tags/language/document_type/provenance), and the switchboard metrics that the production `embed_chunks` performs — a real search-quality regression — and duplicating all of it violates project FP-2 (unify-to-prevent-drift). **Locked decision (Chris):** Case 3 (and resurrection) call the production `embed_chunks` (promoted `pub(crate)`) as the SINGLE embed source, then **RE-KEY** each returned point + `ChunkRecord` to the content-key scheme (`id = real_point_id_for(content_key, i)`) and rewrite the branch-lineage payload fields (`branches:[b]` → singular `branch` + `virtual:false` + `content_key` + `file_identity_id` + `state:"present"`) via `apply_lineage_payload`. `run_tier2_tagging` + `inject_component` then run on the re-keyed points before `insert_points_batch`. Cases 1/2 (which do NOT embed) build the same rich payload via `build_chunk_payload` (also promoted `pub(crate)`) without embedding. Net: no regression, one embed path, no duplication. Steps (b)–(e) above are realized by the single `embed_chunks` call + the re-key loop. The "future shared helper" is no longer needed — reuse IS the helper.
