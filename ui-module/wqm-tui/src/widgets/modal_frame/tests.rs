...
fn the_framework_footprint_is_the_size_the_gate_named() {
    ...
    let big = Footprint::Framework.rect(SCREEN);
    ...
    let small = Footprint::Framework.rect(FLOOR);
...
fn only_the_framework_footprint_clears_the_pages_own_header() {
    ...
    for area in [SCREEN, FLOOR] {
        let rect = Footprint::Framework.rect(area);
        assert!(
        ...
        let literal = Footprint::HelpDerived.rect(area);
        assert!(
...
fn the_two_footprint_arms_are_different_windows() {
    let _serial = crate::global_state_lock();
    let _restore = Restore::mocha();
    let framework = Footprint::Framework.rect(SCREEN);
    let literal = Footprint::HelpDerived.rect(SCREEN);
