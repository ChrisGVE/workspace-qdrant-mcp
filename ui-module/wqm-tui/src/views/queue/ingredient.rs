...
fn list_frames() -> Vec<Box<dyn Ingredient>> {
    vec![
        ...
        Box::new(Variant(
            "Selection",
            "Four rows picked with Space, the cursor on one of them: the selection's quiet tint and the `▎` bar, the cursor's own block winning the row the two share, and the count at the right of the dialog row",
            || {
                queue(QueueState {
                    cursor: 3,
