...
/// [`crate::panes::cell::GUTTER`] for its cursor marker, leaving 57. It then spends `Bch` 3 +
...
fn dropping_pts_gives_its_columns_to_the_name() {
    ...
    assert_eq!(
        bch,
        crate::panes::cell::GUTTER + NAME_WIDTH + 1,
        "the Name column is {NAME_WIDTH} wide with one gap after it: {header:?}"
    );
    assert!(
        bch - 1 - crate::panes::cell::GUTTER > NAME_WIDTH_WITH_PTS,
