...
try:
    ...
    print("Make sure you're running from the project root and have installed the package")
    sys.exit(1)
...
class BenchmarkScenario:
    ...
    def __init__(self, name: str, collection_name: str, chunk_size: int, includes_oss: bool):
        self.name = name
...
class AuthoritativeBenchmark:
    ...
    async def initialize(self):
        ...
        try:
            ...
            console.print(f"✅ Connected to Qdrant at {self.config.qdrant.url}", style="green")
            
        except Exception as e:
            console.print(f"❌ Failed to connect to Qdrant: {e}", style="red")
            console.print("💡 Make sure Qdrant is running: docker run -p 6333:6333 qdrant/qdrant", style="yellow")
            raise
    ...
    async def download_oss_projects(self) -> None:
        ...
        with Progress(
            ...
            for project in self.oss_projects:
                ...
                if project_dir.exists():
                    console.print(f"  ✅ {project.name} already exists, skipping", style="green")
                ...
                try:
                    ...
                    console.print(f"  ✅ {project.name} downloaded successfully", style="green")
                    progress.update(task, completed=100)
                    
                except Exception as e:
                    console.print(f"  ❌ Failed to download {project.name}: {e}", style="red")
        ...
        for project in self.oss_projects:
            if project.local_path and project.local_path.exists():
                size = sum(f.stat().st_size for f in project.local_path.rglob('*') if f.is_file())
            ...
            for scenario in self.scenarios:
                ...
                try:
                    ...
                    cmd = [
                        ...
                        "--chunk-size", str(scenario.chunk_size),
                        "--chunk-overlap", "200", 
                        "--concurrency", "5",
                    ...
                    if result.returncode == 0:
                        ...
                        for line in output_lines:
                            if "Documents created:" in line:
                                scenario.document_count = int(line.split(':')[1].strip().replace(',', ''))
                            elif "Text chunks:" in line:
                                scenario.chunk_count = int(line.split(':')[1].strip().replace(',', ''))
                        
                        console.print(f"  ✅ {scenario.name}: {scenario.document_count} docs, {scenario.chunk_count} chunks", style="green")
                    else:
                        console.print(f"  ❌ {scenario.name}: Ingestion failed", style="red")
                        logger.error(f"Ingestion failed for {scenario.name}: {result.stderr}")
    ...
    def _generate_test_queries(self) -> List[Dict[str, Any]]:
        ...
        symbol_queries = [
            "search_workspace", "QdrantClient", "HybridSearchEngine",
            "initialize", "process_documents", "calculate_metrics", 
            "TestDataCollector", "AuthoritativeBenchmark"
        ]
        ...
        exact_queries = [
            "async def", "import asyncio", "class ", "def __init__",
            "pytest.fixture", "assert ", "return None"
        ]
    ...
    async def _test_search_type(self, scenario: BenchmarkScenario, search_type: str, 
        ...
        relevant_queries = [q for q in queries if q['type'] == search_type or search_type == 'hybrid']
    ...
    async def _perform_search(self, collection: str, query: str, search_type: str) -> List[Dict[str, Any]]:
    ...
    def _calculate_search_quality(self, query: Dict[str, Any], 
                                results: List[Dict[str, Any]]) -> Tuple[float, float, float]:
        ...
        for result in results:
            ...
            if query['type'] == 'symbol':
                ...
                is_relevant = (
                    f"def {query_text}" in content or
                    f"class {query_text}" in content or 
                    f"{query_text}(" in content
                )
    ...
    def generate_comprehensive_report(self) -> None:
        ...
        for scenario in self.scenarios:
            if scenario.document_count:
                scenario_data = self.benchmark_results['scenarios'].get(scenario.name, {})
                ...
                if search_types:
                    precisions = [st.get('avg_precision', 0) for st in search_types.values()]
                    recalls = [st.get('avg_recall', 0) for st in search_types.values()]
                    avg_precision = statistics.mean([p for p in precisions if p > 0]) if precisions else 0
                    avg_recall = statistics.mean([r for r in recalls if r > 0]) if recalls else 0
                ...
                summary_table.add_row(
                    ...
                    f"{scenario.ingestion_time:.1f}s" if scenario.ingestion_time else "N/A",
                    f"{avg_precision:.3f}",
                    f"{avg_recall:.3f}"
                )
    ...
    def _print_performance_comparison(self) -> None:
        ...
        for chunk_size in self.chunk_sizes:
            ...
            if project_data and mixed_data:
                ...
                precision_drop = ((project_precision - mixed_precision) / project_precision * 100) if project_precision > 0 else 0
                recall_drop = ((project_recall - mixed_recall) / project_recall * 100) if project_recall > 0 else 0
    ...
    def _print_chunk_size_analysis(self) -> None:
        ...
        for chunk_size in self.chunk_sizes:
            ...
            if scenario_data:
                ...
                index_size = scenario.chunk_count if scenario and scenario.chunk_count else 0
        ...
        best_precision = max(chunk_analysis.values(), key=lambda x: x['precision'])['precision'] if chunk_analysis else 0
        best_recall = max(chunk_analysis.values(), key=lambda x: x['recall'])['recall'] if chunk_analysis else 0
        best_speed = min((v for v in chunk_analysis.values() if v['response_time'] > 0), key=lambda x: x['response_time'], default={'response_time': 0})['response_time'] if chunk_analysis else 0
        ...
        for chunk_size in self.chunk_sizes:
            ...
            elif data.get('response_time', float('inf')) <= best_speed * 1.1 and best_speed > 0:
        ...
        if chunk_analysis:
            best_chunk_size = max(chunk_analysis.keys(), key=lambda k: chunk_analysis[k]['precision'] * chunk_analysis[k]['recall'])
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Authoritative workspace-qdrant-mcp benchmark")
    parser.add_argument("--skip-oss", action="store_true", help="Skip OSS project download")
    parser.add_argument("--chunk-sizes", nargs="+", type=int, default=[500, 1000, 2000, 4000], 
